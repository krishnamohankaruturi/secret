// It is imperative that the global context is named ‘qtiCustomInteractionContext’. The
// global JavaScript object allows hooks to initiate communication.
qtiCustomInteractionContext = {
		
  teiQuestionNumber : new Array(),
  ...
  getTEIHtmlNode: function(questionNumber) { 
	  //console.log('getTEIHtmlNode questionNumber:' + questionNumber); 
	  if(this.teiQuestionNumber.length > 0 && this.teiQuestionNumber[questionNumber])
		  return this.htmlNodes[this.teiQuestionNumber[questionNumber]];
	  else
		  return null;
  },
  updateTEI: function(questionNumber, response) { 
	  //console.log('updateTEI questionNumber:' + questionNumber);  
	  this.interactions[this.teiQuestionNumber[questionNumber]].initialize(this.teiQuestionNumber[questionNumber], this.htmlNodes[this.teiQuestionNumber[questionNumber]], null, response);
	  this.htmlNodes[this.teiQuestionNumber[questionNumber]] = this.interactions[this.teiQuestionNumber[questionNumber]].getSerializedState();
  },
  getTEIScript: function(questionNumber) { 
	  //console.log('getTEIScript questionNumber:' + questionNumber); 
	  if(this.teiQuestionNumber.length > 0 && this.teiQuestionNumber[questionNumber])
		  return this.interactions[this.teiQuestionNumber[questionNumber]];
	  else
		  return null;
  },
  getTEIId: function(questionNumber) { 
	  //console.log('getTEIId questionNumber:' + questionNumber); 
	  return this.teiQuestionNumber[questionNumber];
  },
  setTEIId: function(questionNumber, teiItemId) { 
	  //console.log('setTEIId questionNumber, teiItemId:' + questionNumber + ',' + teiItemId);   
	  this.teiQuestionNumber[questionNumber] = teiItemId;
  },
   ...
   notifyReady : function(hook_id) {
     ...
     console.log('Interaction Ready:' + hook_id + '-' + this.interactions[hook_id].getTypeIdentifier());
   },
   ...
   notifyDone : function(hook_id) {
     ...
     console.log('Finished interaction:' + hook_id + '-' + this.interactions[hook_id].getTypeIdentifier());
  }
 };
