...
var fcs=false; //dlm-added to know focus of the input element
var graphs=[];
...
//dlm - these 3 random things not used..can delete them
    ...
    if(window && window.app && window.app.variables){
        for(i in vars){
            if(vars.hasOwnProperty(i)){
                if(i=="e" || i=="pi"){
                    throw(MessageStrings["protected"]); // This may be MessageStrings.protected
                    return;
                }
...
var Graph=function(n,disabled){
  ...
  if(n){
    ...
    //Sample function
	//dlm: commented out to remove adding random functions
    latex="";//randfunc();
  }
  var t={};
    var c=compile(latex);
...
app.add=function(n,disabled){
  ...
  graphs.push(graph);
    //track("New"); //dlm-remove
    app.ui.refresh();
    //console.log("focus element:"+fcs);
};
app.png=function(){
    app.ui.png();
};
...
app.refresh=function(changes){
	...
	//console.log("scalex:"+width /(xmx-xmn));
    //console.log("scaley:"+height /(ymx-ymn));
	//app.ui.set_scale(width /($("#xmax").val()-$("#xmin").val()),height /($("#ymax").val()-$("#ymin").val()));
};
...
app.locale_updated = function(locale) {
  ...
  script.src = '/js/tools/graphingCalculator/langs/' + app.locale.toLowerCase() + '.js';
  document.getElementsByTagName('head')[0].appendChild(script);
};
app.changexy=function(){ //////////// dlm added for xmin - xmax
	if($('#xmin').val() && $('#xmax').val() && $('#ymin').val() && $('#ymax').val()){
		if($('#xmin').val() < $('#xmax').val() && $('#ymin').val() < $('#ymax').val()){
			app.ui.showArea($('#xmin').val(),$('#xmax').val(),$('#ymin').val(),$('#ymax').val());
		}
	}
};
app.init=function (){
    ...
    ////app.locale_updated();
	//dlm: commented out - looks unneccessary
	...
	console.log("4..");
		...
		console.log("6..");
        //app.add("\\frac{d}{dx}\\left(sin\\left(x\\right)+log\\left(x+1\\right)\\right)"); //dlm: removed this default expression
    }*/
	//dlm commented out - looks unnecessary
