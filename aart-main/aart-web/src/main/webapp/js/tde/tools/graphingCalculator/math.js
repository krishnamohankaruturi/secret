...
Number.prototype.format=function(digits) {
    ...
    if (!num) {
        ...
        if (( log(num)   )%1 == 0) {
            ...
            var exptext="Ã¢ï¿½Â°Ã‚Â¹Ã‚Â²Ã‚Â³Ã¢ï¿½Â´Ã¢ï¿½ÂµÃ¢ï¿½Â¶Ã¢ï¿½Â·Ã¢ï¿½Â¸Ã¢ï¿½Â¹";
            return "e"+((abs(exponent)<10)?((exponent<0?"Ã¢ï¿½Â»":"")+exptext[abs(exponent)]):"^"+exponent);
        }
    }
    if (digits === undefined) {
        return num.toString()
...
//parses brackets recursively and returns an expression
    ...
    //TODO: -- -> +
    e=e.replace(/Ã¢Ë†Å¾/g,"Infinity");
    e=e.replace(/\.([^\d]|$)/g,"*$1");
    e=e.replace(/([\d]+(\.[\d]+)?)([^\+\-\*\/\^\:\(\)\d\=\<\>\.!])/g,"$1*$3");
    
    //TODO: Following line is a bit hacky. Specifications need be made to clear things up.
    e=e.replace(/([xyzÃ�â‚¬Ã�â€¢])([exyzÃ�â‚¬Ã�â€¢])/g,"$1*$2");

	e=e.replace(/\^([\d]+)\(/g,"^$1:(");
    
    e=e.replace(/max\(/g,"(max)(");
    
    e=e.replace(/([xyzÃ�â‚¬Ã�â€¢\dÃ¢Ë†Â«])\(/g,"$1*(");
    e=e.replace(/\(max\)/g,"max");
    
    e=e.replace(/Ã¢Ë†Â«([^\*])/g,"Ã¢Ë†Â«*$1");
    e=e.replace(/([xyzÃ�â‚¬Ã�â€¢\d\.])Ã¢Ë†Â«/g,"$1*Ã¢Ë†Â«");
	e=e.replace(/([^\+\-\*\/\^\:\(\)\d\=\<\>])\(/g,"$1:(");
...
functions="u_d,sin,cos,tan,sec,cot,csc,cosec,acosh,asinh,atanh,cosh,sinh,tanh,cosech,csch,sech,coth,log,exp,pow,Gamma,sinc,sqrt,W,fact,bellb,Zeta,u,doublefact,signum,asin,acos,atan,arcsin,arccos,arctan,tg,ln,abs,floor,round,ceil,atan2,random,min,max,clear,text,shaw,delta,ÃŽâ€œ,Ã�Ë†,diff,int,ÃŽÂ¶".split(",");
...
var known_derivatives={
    ...
    "asin":p("1/sqrt(1-x^2)"),
    "acos":p("-1/sqrt(1-x^2)"),
    "fact":p("ÃŽâ€œ(x+1)*Ã�Ë†(x+1)"),
    "doublefact":p("0.25*x!!*(2*Ã�Ë†(x/2+1)+pi*log(2/pi)*sin(pi*x)+log(4))"),
    "Gamma":p("ÃŽâ€œ(x)*Ã�Ë†(x)"),
    "ÃŽâ€œ":p("ÃŽâ€œ(x)*Ã�Ë†(x)")
};
