...
(function($, undefined) {
	...
	jPlayerPlaylist = function(cssSelector, playlist, options) {
		...
		$(this.cssSelector.jPlayer).bind($.jPlayer.event.ended, function(event) {
			//dlm - added : tts unhighlight b4 playing next audio
			if($(this).attr("id") == 'ttsAudio' || $(this).attr("id") == 'ttsAudio_direction') {
				tagHighlighter.unhighlight();
				if((self.current+1) == self.playlist.length) {
					$('.tde-controls-ttsPlay, .ttsPlay').addClass('play').removeClass('pause');
					//$('.tde-controls-ttsPlay').text('play');
					$('.tde-controls-ttsPlay img').attr('src', '/images/tts-read.png');
				}
			}
			self.next();
			if($(this).attr("id") == 'auditoryCalmingPlayer') {
				//var index = (self.current + 1 < self.playlist.length) ? self.current + 1 : 0;
				$('#auditoryCalming .player-title').html(self.playlist[self.current].title);
			}
		});
	...
	jPlayerPlaylist.prototype = {
		...
		_createItemHandlers: function() {
			...
			$(this.cssSelector.playlist + " a." + this.options.playlistOptions.itemClass).die("click").live("click", function() {
				...
				$(this).blur();
				if(self.cssSelector.jPlayer == '#auditoryCalmingPlayer') {
					$('#auditoryCalming .player-title').html(self.playlist[self.current].title);
				}
				return false;
			});
		...
		play: function(index) {
			...
			if(0 <= index && index < this.playlist.length) {
				if(this.playlist.length) {
					this.select(index);
					// dlm-add : highlight while playing for tts
					if((this.cssSelector.jPlayer == '#ttsAudio' || this.cssSelector.jPlayer == '#ttsAudio_direction') && !this.isPaused) {
						if(this.playlist[index].contentGroup.grouptype == 'task') {
							ttsAccommodation.openScrollPassageQuestion(this.playlist[index].contentGroup.htmlElementId);
						}
						tagHighlighter.highlight(this.playlist[index].contentGroup);//dlm : highlight and play
					}
				...
				// dlm-add : highlight while playing for tts
				if((this.cssSelector.jPlayer == '#ttsAudio' || this.cssSelector.jPlayer == '#ttsAudio_direction') && !this.isPaused) {
					if(this.playlist[0].contentGroup.grouptype == 'task') {
						ttsAccommodation.openScrollPassageQuestion(this.playlist[0].contentGroup.htmlElementId);
					}
					tagHighlighter.highlight(this.playlist[0].contentGroup); //dlm : highlight and play
				}
				$(this.cssSelector.jPlayer).jPlayer("play");
				//isPaused = false;
			}
		...
		next: function() {
			...
			if(this.loop) {
				...
				if(index > 0) {
					if((this.cssSelector.jPlayer == '#ttsAudio' || this.cssSelector.jPlayer == '#ttsAudio_direction') && this.playlist[index].contentGroup.readAloudAccommodation.delay != undefined) { //dlm-added : time delay between audios
						var self = this;
						setTimeout(function() {
							self.play(index);
						}, this.playlist[index].contentGroup.readAloudAccommodation.delay);
					} else {
						this.play(index);
					}
				} else if (index == 0 && (this.cssSelector.jPlayer == '#ttsAudio' || this.cssSelector.jPlayer == '#ttsAudio_direction')) { //dlm-added : go back to first audio in playlist after playing once
					this.select(0);
					$('.ttsPlay').removeClass('pause').addClass('play');
				}
			}
		},
		previous: function() {
