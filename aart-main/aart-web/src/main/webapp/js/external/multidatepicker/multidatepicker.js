	...
	var	views	= {
		...
		tpl		= {
			...
			head	: function (d) {
				...
				return '<input type="hidden"class="hiddenSavetype" id="hiddenSavetype" value="gridRow"><input type="hidden"class="hiddenValuesDatePicker" ><div class="divLabelwidth" id="divLabelwidth"></div><div class="divLabelwidth" id="gradeLabelwidth"></div><div class="divLabelwidth" id="projectionTypeLabelwidth"></div>' +									
				    '<div class="pmu-instance">' +
					'<nav>' +
						'<div class="pmu-prev pmu-button">' + d.prev + '</div>' +
						'<div class="pmu-month pmu-monthbutton" />' +
						'<div class="pmu-next pmu-button">' + d.next + '</div>' +
					'</nav>' +
					'<nav class="pmu-day-of-week">' + result + '</nav>' +
				'</div>'+
				'<div class="divbtn" ><button class="multicalenCancelbtn" onclick="multicalenCancelbtn();" >Cancel</button><button class="multicalenSavebtn" onclick="multicalenSavebtn();" >Save</button></div>';
			},
			body	: function (elements, container_class_name) {
				var result	= '';
				for (var i = 0; i < elements.length; ++i) {
				/*if(elements[i].class_name.indexOf('pmu-saturday') > -1){
					result	+= '<div class="' + elements[i].class_name + ' weekendDisable">' + elements[i].text + '</div>'
				}
				else if(elements[i].class_name.indexOf('pmu-sunday') > -1){
					result	+= '<div class="' + elements[i].class_name + ' weekendDisable">' + elements[i].text + '</div>'					
				}	*/
				if(elements[i].class_name.indexOf('pmu-not-in-month') > -1){
				     result += '<div class="pmu-not-in-month opacity-not-in-month">' + elements[i].text + '</div>'     
				}
				else{
