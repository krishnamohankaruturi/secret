...
<xsl:stylesheet version="1.1" xmlns:saxon="http://saxon.sf.net/"
	...
	<xsl:template name="additionalResources"> 
	  ...
	  <xsl:choose>
		<xsl:when test="(reportDetails/data/contentAreaCode = 'ELA' or reportDetails/data/contentAreaCode = 'M') and (reportDetails/data/gradeCode = '10' or reportDetails/data/gradeCode = '8')">
												...
												<xsl:choose>
													<xsl:when test="(reportDetails/data/contentAreaCode = 'ELA' or reportDetails/data/contentAreaCode = 'M')">
														...
														<fo:block>
												  		...
												  		<xsl:choose>
															<xsl:when test="(reportDetails/data/gradeCode = '10' or reportDetails/data/gradeCode = '8') ">
												  					<xsl:call-template name="ACTSectionTable"/>
												  			</xsl:when>												  			
												  		</xsl:choose>												  		
													</xsl:when>	
	...
	<xsl:template name ="ACTSectionTable" >
			<xsl:if test="(reportDetails/data/contentAreaCode = 'ELA' or reportDetails/data/contentAreaCode = 'M')">
    				<xsl:variable name="totallevels" select="reportDetails/data/noOfActlevels"/>
													...
													<fo:table-body>	
															<fo:table-row>	
																...
																border-right-style="solid" keep-together.within-column="always"  number-rows-spanned="{$totallevels}" >
																	...
																	<xsl:if test="(reportDetails/data/gradeCode = '8')">
																	<fo:block font-size="8pt" font-family="Verdana" >																		
																		<xsl:text>To get an idea of how your eighth grade student may perform on the ACT based on this KAP score, refer to this chart. For more information, go to </xsl:text>
																		<fo:inline font-weight="bold"><xsl:text>ksassessments.org/act</xsl:text></fo:inline>
																		<xsl:text>.</xsl:text>
																	</fo:block>
																	</xsl:if>
																</fo:table-cell>
																<xsl:for-each select="reportDetails/data/actScoringDescription/string">
															...
															<xsl:for-each select="reportDetails/data/actScoringLevel/edu.ku.cete.domain.report.ActScoringLevel">													
															<xsl:variable name="levelId" select="levelId"/>															
															<fo:table-row>	
															<xsl:for-each select="actScoringDescriptions/edu.ku.cete.domain.report.ActScoringDescription">															  														  
																<fo:table-cell 
																	padding-left="3pt" padding-right="2pt" padding-top="2pt"
																	padding-bottom="2pt" border-top-style="solid"
																	border-bottom-style="solid" border-left-style="solid">
																	<fo:block font-size="8pt" font-family="Verdana">
																		<xsl:if test="position() = 1"><xsl:text>Level </xsl:text><xsl:value-of select='$levelId'/><xsl:text>:</xsl:text></xsl:if> <xsl:value-of select="minvalue" /><xsl:text>-</xsl:text><xsl:value-of select="maxvalue" />
																	</fo:block>
																</fo:table-cell>
																</xsl:for-each>																				
															</fo:table-row>																				
										...
										<fo:table-body>
											<fo:table-row>
												<fo:table-cell text-align="left">
													...
													<xsl:choose>
													<xsl:when test="(reportDetails/data/contentAreaCode = 'ELA' or reportDetails/data/contentAreaCode = 'M') and (reportDetails/allGrades/string[contains(substring-after(., 'Grade '),'10')]  or reportDetails/allGrades/string[contains(substring-after(., 'Grade '),'8')] )">
														<xsl:call-template name="ACTSectionTable"/>
