		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format"
			...
			<fo:page-sequence master-reference="allPages">
							...
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell text-align="start" wrap-option="no-wrap">
										<fo:block color="#A7A2A2">District Detail Report</fo:block>
									</fo:table-cell>
									<fo:table-cell text-align="center">
										<fo:block color="#A7A2A2">District: <xsl:value-of select="reportDetails/data/districtName"/></fo:block>
									</fo:table-cell>
				...
				<fo:flow flow-name="xsl-region-body">
					<xsl:variable name="medianDistrictScoreCheck">
						...
						<xsl:when test="$medianDistrictScoreCheck > 0">
										...
										<fo:table table-layout="fixed">
											...
											<fo:table-body>
												<fo:table-row>
													<fo:table-cell>
														<fo:block>
															<xsl:variable name="districtScore">
																<xsl:call-template name="lookup-median-se-sc">
																	<xsl:with-param name="paramOrgId" select="reportDetails/data/districtId"/>
																	<xsl:with-param name="paramType" select="'SCORE'"/>
																</xsl:call-template>
															...
															being the highest level. The district's median score is in Level <xsl:call-template name="lookup-level-number-byscore">
																<xsl:with-param name="paramScore" select="$districtScore"/>
								...
								<xsl:if test="count(reportDetails/data/subscoreBuckets) &gt; 0">
									<fo:block break-before="page">
										...
										<fo:block margin-top="4mm" font-weight="bold" font-size="12pt" text-align="center">District Sub-Scores and Claims</fo:block>
										<fo:block margin-top="3mm">
											This chart shows the district’s performance on specific areas of the <xsl:value-of select="reportDetails/data/gradeName"/><xsl:text>&#160;</xsl:text><xsl:value-of select="reportDetails/data/contentAreaName"/> test as well as the performance of the <xsl:call-template name="lower-case"><xsl:with-param name="x" select="reportDetails/data/gradeName"/></xsl:call-template> students in the district and state. The bracket on either side of the bold score line represents the standard error, or how much a student's performance might vary if the student took many equivalent versions of the test.
										</fo:block>
										<fo:block margin-top="3mm">
			...
			<fo:table-body>
				<fo:table-row>
					<fo:table-cell padding="2px">
						<fo:block color="#3784C6" font-weight="bold" font-size="11pt">District Detail Report</fo:block>
						<xsl:call-template name="headerDistrictName"/>
					</fo:table-cell>
					<fo:table-cell text-align="center">
						<xsl:call-template name="headerLogo"/>
					</fo:table-cell>
					<fo:table-cell text-align="end" padding-left="2px" padding-top="2px" padding-bottom="2px">
						<fo:block-container>
