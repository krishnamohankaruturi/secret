		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format"
			...
			<fo:layout-master-set>
				...
				<fo:simple-page-master master-name="lastPage"
					page-height="27.94cm" page-width="21.59cm" margin="1.27cm">
					<fo:region-body margin-top="7mm" margin-bottom="15mm" />
					<fo:region-before region-name="xsl-region-before-lastPage" extent="7mm" />
					<fo:region-after region-name="xsl-region-after-lastPage" extent="24mm" />  <!-- display-align="lastafter" -->
				</fo:simple-page-master>
								...
								<fo:table-row>
									...
									<fo:table-cell text-align="right">
										<fo:block font-size="8pt" text-align="right"
											margin-bottom="8mm">
											<fo:block-container height="100px" width="100px"
												position="absolute">
				...
				<fo:flow flow-name="xsl-region-body">
	 			...
	 			<fo:block>
						<fo:block margin-top="-25pt" space-after="5pt">
							<fo:leader leader-pattern="rule" leader-length="100%" rule-thickness="0.2pt"
								rule-style="solid" />
						</fo:block>
						<fo:block margin-top="-25pt" font-size="9pt">
							...
							<xsl:if test="reportDetails/data/assessmentProgramCode = 'AMP'">
								This report has information about the school's Alaska Measures
								of Progress (AMP) test scores. The AMP measures students'
								understanding of the Alaska English Language Arts and
								Mathematics Standards in grades 3-10 using questions that ask
								students to select the right answer, sort items, create graphs,
								and label pictures. For sample test questions, see
								http://amp.cete.us/alaskatpt.
							</xsl:if>
							</xsl:if>
						</fo:block>
						<xsl:if test="reportDetails/data/assessmentProgramCode = 'AMP'">
							<fo:block margin-top="4mm" font-weight="bold">
								Median School
								<xsl:if test="reportDetails/data/assessmentProgramCode = 'KAP'">
							...
							<fo:block margin-top="3mm">
								This chart compares the school’s overall
								<xsl:value-of select="reportDetails/data/contentAreaName" />
								test scores by grade to the median
								<xsl:value-of select="reportDetails/data/contentAreaName" />
								test scores in the district and state.
							</fo:block>
						</xsl:if>
						<xsl:if test="reportDetails/data/assessmentProgramCode = 'KAP'">
							<fo:block font-weight="bold" font-family="Verdana"
								font-size="12pt" text-align="center" margin-top="10pt">
								<xsl:if test="reportDetails/data/assessmentProgramCode = 'KAP'">
									<xsl:if test="reportDetails/data/contentAreaCode = 'ELA'">
									...
									<xsl:if test="reportDetails/data/contentAreaCode = 'M'">
										<!-- Mathematics:  -->Median School, District, and State Performance
									</xsl:if>
									<xsl:if test="reportDetails/data/contentAreaCode = 'SS'">
							...
							<fo:block text-align="center"
								margin-top="4px">
	 							<xsl:call-template name="median-table-legend" />
							</fo:block>
						</xsl:if>
		...
		<!-- 2nd page -->
	 				...
	 				<xsl:if test="reportDetails/data/assessmentProgramCode = 'AMP'">
						<fo:block break-before="page">
							...
							<fo:block text-align="center" margin-right="50mm" padding="2px" font-size="8pt" font-style="italic"
								color="#939597">
			...
			<fo:table-body>
					...
					<xsl:if test="reportDetails/data/contentAreaCode = 'SS'">
							...
							<fo:block text-align="center" margin-left="-2.5mm"
								margin-top="4px">
								<xsl:call-template name="levels-table-legend" />
					</fo:block>
					<fo:block text-align="center" padding="2px" font-size="8pt"	font-style="italic" margin-left="9pt">
						...
						<xsl:choose>
							<xsl:when test="reportDetails/data/contentAreaCode != 'SS'">
								<xsl:call-template name="rating_icon_legand">
									...
									select="'School'"></xsl:with-param>
								</xsl:call-template>
								<fo:block margin-top="4pt" space-after="5pt">
									<fo:leader leader-pattern="rule" rule-thickness="0.5pt"  background-color="#939597" leader-length="100%"
										rule-style="solid" />
								</fo:block>
								<fo:block>
						...
						<xsl:if test="reportDetails/data/contentAreaCode != 'SS'">	
							<fo:block-container keep-together.within-page="always">
								<fo:block font-size="9pt" text-align="left" font-weight="bold"
									margin-top="6pt">
									<xsl:text>Your School’s Performance</xsl:text>
								</fo:block>
			...
			<fo:table-body>
				<fo:table-row>
					...
					<fo:block font-family="Verdana" font-weight="bold"
							...
							wrap-option="no-wrap">
							<xsl:call-template name="summaryheaderSchoolName" />
						</fo:block>
						<fo:block-container>
					...
					<fo:table-cell text-align="end">
						<fo:block-container display-align="center">
								<fo:block text-align="center" color="#25408f" font-family="Verdana"
		...
		<fo:table table-layout="fixed">
			...
			<fo:table-body>
				<fo:table-row>
					...
					<fo:table-cell>
						<fo:block font-size="8pt" text-align="left" margin-top="33pt">
				...
				<fo:table-body>
					<fo:table-row>
						<fo:table-cell text-align="start" wrap-option="no-wrap">
							<fo:block>SCHOOL REPORT</fo:block>
						</fo:table-cell>
						<fo:table-cell text-align="center">
							<fo:block>
								<!-- DISTRICT:
						...
						<fo:table-cell text-align="end" wrap-option="no-wrap">
							<fo:block>
								SCHOOL:
								<xsl:value-of select="translate(reportDetails/data/summaryHeaderSchoolName,'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')" />
							</fo:block>
						</fo:table-cell>
					</fo:table-row>
				</fo:table-body>
