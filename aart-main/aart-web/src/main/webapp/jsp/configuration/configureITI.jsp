...
<c:if test="${hasQCPermission}">
	...
	<div><div id="itiByStateDiv" class="form-fields">
		<label for="ITIpermissionLabel" class="field-label">Enable ITI By State:</label>
		<div id="ITIbyStatePopup" hidden='hidden'>
			<form>
				<div style="float:left;padding-right:20px;">
		  		...
		  		<div id="popupErrorMsg" hidden="hidden"></div>
			</form>
		...
		<div>
			<table id="iti-states-table" class="gridtable">
				<thead>
					<tr>
						<td><input type="checkbox" id="selectAll" style="float:left;">State</td>
...
$(function () {
		$('.slider-button-recommendedLevel').click(function(){
		...
		$('#editWindow').click(function(e){
			...
			if ($("#iti-states-table > tbody input:checkbox:checked").length <= 0){
				...
				if($("#iti-states-table > tbody input:checkbox:checked").length == 1){
					currentStartDate = $("#iti-states-table > tbody input:checkbox:checked").closest("tr").find("td:nth-child(3)").text();
					$("#startDate").val(currentStartDate);
					$("#endDate").val($("#iti-states-table > tbody input:checkbox:checked").closest("tr").find("td:nth-child(4)").text());
				}else{
					$("#startDate").val("");
					$("#endDate").val("");
				}
		...
		success: function(data){
			...
			if(data.records != 0){
			    ...
			    data.sort(function (x,y) {
				      return ((x.cell[2] == y.cell[2]) ? 0 : ((x.cell[2] > y.cell[2]) ? 1 : -1 ));
			    });
				for(var i = 0; i < data.length; i++){
					...
					$('#iti-states-table > tbody:last-child').append('<tr><td><input type="checkbox" style="float:left;" id = ' + id + '>' + org + '</td><td>' + status + ' </td><td>' + begin + ' </td><td>' + end + ' </td></tr>')
				}
