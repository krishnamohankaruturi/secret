...
public class BatchAlternateOrganizationGrfCalculationProcess implements ItemProcessor<AlternateAggregateReport,Object>
{
	@Autowired 
	...
	final static Log logger = LogFactory.getLog(BatchAlternateOrganizationGrfCalculationProcess.class);
    
	@Override
    public AlternateAggregateReport process(AlternateAggregateReport grfData) throws Exception {
		logger.debug("Inside BatchAlternateOrganizationGrfCalculationProcess for Organization Report Calculation - Org Id - "+ grfData.getSchoolId());
		AlternateAggregateReport aggregateGRFReportData = uploadGrfFileService.getGeneralResearchDataByOrganizationId(grfData.getSchoolId(), reportYear, assessmentProgramId, stateId);
		return doCalculationGrfDataForOrganizationReport(aggregateGRFReportData);		
    }

	private AlternateAggregateReport doCalculationGrfDataForOrganizationReport(AlternateAggregateReport aggregateGRFReportData){	 
			 ...
			 for(AlternateAggregateStudents alternateAggregateStudent : aggregateGRFReportData.getAlternateAggregateStudents()){
				 ...
				 if(alternateAggregateStudent.getAlternateAggregateSubject().size()>0){			 
					 for(AlternateAggregateSubject alternateAggregateSubject : alternateAggregateStudent.getAlternateAggregateSubject()){
						 ...
						 else if(alternateAggregateSubject.getInvalidationCode().longValue()!=CommonConstants.GRF_LEVEL_1.longValue() && alternateAggregateSubject.getPerformanceLevel().longValue()==CommonConstants.GRF_LEVEL_9.longValue()){
