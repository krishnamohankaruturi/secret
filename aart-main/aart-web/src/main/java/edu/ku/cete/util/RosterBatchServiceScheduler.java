...
@Component
public class RosterBatchServiceScheduler  implements MessageSourceAware{
    /**
     * logger.
     */
    private static final Log LOGGER = LogFactory.getLog(RosterBatchServiceScheduler.class);
	/**
	 * set the metadata.
	 */
	@Autowired
	 ...
	 */
	private RosterBatchServiceTask rosterWebServiceTask;
	/**
	 * email path.
	 */
    @Value("${email.path}")
	...
	public final void initializeWebServiceTask() {
		rosterWebServiceTask = new RosterBatchServiceTask();
		...
		rosterWebServiceTask.setCategoryDao(categoryDao);
