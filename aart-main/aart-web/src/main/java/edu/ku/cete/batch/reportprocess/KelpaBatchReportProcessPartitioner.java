...
public class KelpaBatchReportProcessPartitioner implements Partitioner {

	private final static Log logger = LogFactory .getLog(KelpaBatchReportProcessPartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		...
		//gradeband - gradecourse unit test validation. Mostly no code change. 
						...
						if(CollectionUtils.isNotEmpty(gradeCourses)){ 
							...
							for(GradeCourse gradeCourse: gradeCourses) {
								for(Organization contractingOrg: contractingOrgs) {
									...
									List<ExcludedItems> excludedItems= excludedItemsService.getByAssessmentProgramContentAreaGrade(assessmentProgramId, contentArea.getId(), gradeCourse.getId(), Long.valueOf(contractingOrg.getReportYear()));
