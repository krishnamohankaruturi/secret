...
public class BatchOrganizationSummaryReportPartitioner implements Partitioner {

	private final static Log logger = LogFactory .getLog(BatchOrganizationSummaryReportPartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		logger.info("Enter BatchOrganizationSummaryReportPartitioner partition size : "+gridSize);
		...
		logger.info("batchRegistrationId - " + batchReportProcessId + " , assessments found - " + assessments.size());
		for(Assessment assessment: assessments) {
			...
			logger.info("batchRegistrationId - " + batchReportProcessId + " , Test Type found - " + testTypes.size());
			for(TestType testType: testTypes) {
				List<ContentArea> contentAreas = getContentAreas(testType,assessment);
				if(CollectionUtils.isNotEmpty(contentAreas)){
					logger.info("batchRegistrationId - " + batchReportProcessId + " , contentAreas found - " + contentAreas.size());
					for (ContentArea contentArea : contentAreas) {
								for(Organization contractingOrg: contractingOrgs) {
									logger.info("processing contracting organization: "+contractingOrg.getId() +" ("+contractingOrg.getDisplayIdentifier()+")");
									ExecutionContext context = new ExecutionContext();
									context.put("contentArea", contentArea);
									context.put("gradeId", gradeCourseId);
		...
		logger.info("Created "+partitionMap.size()+" partitions.");
		return partitionMap;
	}
