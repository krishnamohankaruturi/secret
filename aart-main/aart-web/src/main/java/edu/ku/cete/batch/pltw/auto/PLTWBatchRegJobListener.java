 ...
 *
 * Aug 28, 2018 4:18:07 PM
 */
@Component("pltwBatchRegJobListener")
public class PLTWBatchRegJobListener implements JobExecutionListener {
	...
	public void beforeJob(JobExecution jobExecution) {
		...
		if(null != params.getLong("testingProgramId") && params.getLong("testingProgramId") != 0) {
			brRecord.setTestingProgram(params.getLong("testingProgramId"));
		}
...
*/		if(null != params.getLong("operationalTestWindowId") && params.getLong("operationalTestWindowId") != 0) {
		...
		if(null != params.getLong("enrollmentMethodId") && params.getLong("enrollmentMethodId") != 0) {
			brRecord.setAutoEnrollmentMethodId(params.getLong("enrollmentMethodId"));
		}
		...
		jobExecution.getExecutionContext().put("jobDashboardMessages",new CopyOnWriteArrayList<DashboardMessage>());
	...
	public void afterJob(JobExecution jobExecution) {
		...
		if(null != brRecord) {
			...
			for(StepExecution sExecution: sExecutions) {
				if(sExecution.getStepName().startsWith("partitioned")) {
					...
					successCount = sExecution.getWriteCount() - writeSkipCount;
			...
			List<DashboardMessage> jobDashboardMessages = (List<DashboardMessage>) jobExecution.getExecutionContext().get("jobDashboardMessages");
			for(DashboardMessage dbMsg: jobDashboardMessages) {
				if(CollectionUtils.isNotEmpty(dashboardMessageService.getDashboardMessages(dbMsg))) {
					dashboardMessageService.updateDashboardMessages(dbMsg);
				}else {
					dashboardMessageService.insertDashboardMessage(dbMsg);
				}
		...
		jobExecution.getExecutionContext().remove("jobDashboardMessages");
