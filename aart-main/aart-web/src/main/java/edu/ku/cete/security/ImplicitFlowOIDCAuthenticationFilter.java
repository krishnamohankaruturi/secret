...
/**
 * This class was created to handle implicit flow for OpenId Connect.
 * 
 * The main difference from the OIDCAuthenticationFilter is the handleAuthorizationCodeResponse
 * method is not present and the handleIDTokenResponse has been included.
 * 
 * The reason for this difference is in the implicit flow an id_token is returned instead of the 
 * code.  In the implicit flow all steps are combined to return the id_token without making a separate
 * request for the authentication code.
 * 
 * @author craigshatswell_sta
 *
 */
public class ImplicitFlowOIDCAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	 ...
	 */
	public ImplicitFlowOIDCAuthenticationFilter() {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
		...
		} else if (!Strings.isNullOrEmpty(request.getParameter("id_token"))) {

			// we got back the code, need to process this to get our tokens
			Authentication auth = handleIDTokenResponse(request, response);
			return auth;
	...
	/**
	 * This method handles the response that includes the access_token,
	 * id_token and possibly the refresh token.
	 * 
	 * @param request
	 *            The request from which to extract parameters and perform the
	 *            authentication
	 ...
	 *         incomplete.
	 */
	protected Authentication handleIDTokenResponse(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		...
		String idTokenValue = request.getParameter("id_token");
		String refreshTokenValue = request.getParameter("refresh_token");
		
		if (StringUtils.isEmpty(accessTokenValue)) {
			logger.error("Auth Endpoint did not return an access_token");
			throw new AuthenticationServiceException("Auth Endpoint did not return an access_token");
		}

		if (StringUtils.isEmpty(idTokenValue)) {
			logger.error("Auth Endpoint did not return an id_token");
			throw new AuthenticationServiceException("Auth Endpoint did not return an id_token");
		}
