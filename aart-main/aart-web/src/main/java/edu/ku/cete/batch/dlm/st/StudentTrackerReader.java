...
public class StudentTrackerReader<T> extends AbstractPagingItemReader<T> {

	private final static Log logger = LogFactory.getLog(StudentTrackerReader.class);
	...
	private Collection<? extends T> getUntrackedStudents() {
		return (Collection<? extends T>)studentTrackerService.getUntrackedStudents(contractingOrganization, contentArea, operationalWindowId, getPage() * getPageSize(), getPageSize());
	}
