 ...
 *
 * Sep 27, 2017 3:11:00 PM
 */
public class PredictiveOrganizationSummaryCalcPartitioner implements Partitioner {

	private final static Log logger = LogFactory .getLog(PredictiveOrganizationSummaryCalcPartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		...
		List<TestingProgram> testPrograms = testingProgramService.getByAssessmentProgIdAndTestingProgAbbr(assessmentProgramId, "F");
		
		List<Assessment> assessments = getAssessments(testPrograms.get(0).getId());
					...
					for (ContentArea contentArea : contentAreas) {
						...
						if(CollectionUtils.isNotEmpty(gradeCourses)){ 
							...
							for(GradeCourse gradeCourse: gradeCourses) {
									...
									partitionMap.put(assessment.getAssessmentCode()+"_"+testType.getTestTypeCode()+"_"+contentArea.getAbbreviatedName()+"_"+gradeCourse.getAbbreviatedName(), 
											context);							
							}
						}
					}
