    ...
    FOR studenttest IN (SELECT st.id, st.studentid, st.testid,st.testcollectionid,ts.operationaltestwindowid
      ...
      INNER JOIN studentstestsections sts ON sts.studentstestid = st.id
          ...
          OR (otw.effectivedate <= CURRENT_TIMESTAMP AND ts.windowexpirydate >= CURRENT_TIMESTAMP))
                                AND sts.statusid != timedout_status_id
...
ALTER TABLE batchstudenttrackerreason DROP CONSTRAINT batchstudenttrackerreason_testid_fkey;
ALTER TABLE studentreport DROP CONSTRAINT studentreport_enrollmentid_fk;