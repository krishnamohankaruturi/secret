--ddl/539.sql -- addStudentToRosterWithCourse with return messages
--Author : Rohit Yadav
...
$BODY$
 ...
 BEGIN
   ...
   SELECT INTO teacher_Id (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE uso.organizationid = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(teacher_uniqueCommonId) LIMIT 1);
   ...
   ELSE
    IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
        ...
        error_msg := '<error>'||'Studentid: '||COALESCE(state_student_identifier,'NULL')||'is not found with ayp '||COALESCE(ayp_sch_displayidentifier,'NULL')||','||'attendance '||COALESCE(att_sch_displayidentifier,'NULL')||'in school year '||CAST(COALESCE(schoolyear,0) AS TEXT);
    ELSE
    ...
    error_msg := '<success>'||'Studentid: '||COALESCE(state_student_identifier,'NULL')||'is added to the roster '||COALESCE(roster_name,'NULL');
   ...
   IF((SELECT count(*) FROM roster r WHERE lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id
    ...
    error_msg := '<validation>' || 'No rosters found with roster name:'|| COALESCE(roster_name,'NULL') || ','||'subject:' ||COALESCE(subject_abbrName,'NULL')||','||'attendanceschool:'
     ...
     IF(teacher_Id is null) THEN
         ...
         error_msg := '<error>' || 'Teacher: ' || COALESCE(teacher_uniqueCommonId,'NULL')|| ' is not found in school ' || COALESCE(att_sch_displayidentifier,'NULL');
        ...
        LOOP
          ...
          INSERT INTO domainaudithistory (source,objecttype,objectid, createduserid, createddate, action, objectaftervalues) VALUES ('LOCK_DOWN_SCRIPT', 'ENROLLMENT_ROSTER', enrl_Id, ceteSysAdminUserId, now(),
           ...
           RAISE NOTICE 'Student % is added to the roster %', enrollmentRecord.statestudentidentifier, roster_name;
            ...
            error_msg :=  '<success>' || 'Student ' || COALESCE(enrollmentRecord.statestudentidentifier,'NULL') || ' is added to the roster ' || COALESCE(roster_name,'NULL');
	      ...
	      FOR enrlRecords IN (SELECT * FROM enrollmentsrosters WHERE enrollmentid = enrollmentRecord.id AND rosterid = rosterRecord.id LIMIT 1)
	        LOOP
		      ...
		      'ADD_STUDNET_TO_ROSTER', ('{"rosterId":' || enrlRecords.rosterid || ', "enrollmentId":' ||  enrlRecords.enrollmentid || ',"enrollmentRosterId":' || enrlRecords.id || '}')::json);
                 ...
                 error_msg :=  '<success>' || 'enrollmentid ' || CAST(COALESCE(enrlRecords.enrollmentid,0) AS TEXT)  || ' is added to the roster ' || COALESCE(roster_name,'NULL');
 ...
 BEGIN
   ...
   IF((SELECT count(*) FROM roster r WHERE lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id
     ...
     IF(teacher_Id is null) THEN
         ...
         error_msg := '<error>' || 'Teacher: ' || COALESCE(teacher_uniqueCommonId,'NULL')|| ' is not found in the organization ' || COALESCE(att_sch_displayidentifier,'NULL');
    ...
    FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
        ...
        LOOP
          UPDATE enrollmentsrosters SET activeflag = false,modifieddate=CURRENT_TIMESTAMP, modifieduser = ceteSysAdminUserId
           ...
           error_msg := '<success> Student '|| COALESCE(studentsEnrollemntsRostersRecord.statestudentidentifier,'NULL') || ' is removed from roster ' || COALESCE(roster_name,'NULL');
     ...
     FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
        ...
        LOOP
          UPDATE enrollmentsrosters SET activeflag = false,modifieddate=CURRENT_TIMESTAMP, modifieduser = ceteSysAdminUserId
           ...
           error_msg := '<success> Student with id :'|| COALESCE(studentsEnrollemntsRostersRecord.statestudentidentifier,'NULL') || ' is removed from roster ' || COALESCE(roster_name,'NULL');
       ...
       LOOP
  		...
  		PERFORM inActivateStuTestsTrackerITITestsessions(studentTestsId := stuTestsRecordsInprgsPenUnusedStatus.id , inActiveStuTestSecStatusId := exitStuTestSecsStatus,
           ...
           ELSE
             ...
             error_msg := '<error>' || 'Exit withdrawal date ' ||  cast(coalesce(exitDate,'01/01/1900') as text) || ' is less than the school entry date ' || cast(coalesce(studentEnrollemntRecord.schoolentrydate,'01/01/1900') as text) ;
         END IF;
     END LOOP;
     END IF;
     RETURN error_msg;
...
$BODY$
 ...
 BEGIN
   ...
   ELSE
	...
	error_msg = '<error>'||'Student :'|| COALESCE(statestudent_identifier,'NULL') || 'is not present in state :' || COALESCE(stateDisplayidentifier,'NULL');
	...
	error_msg = '<success>'||'Student:' || COALESCE(statestudent_identifier,'NULL') || 'is enrolled into School:' || COALESCE(attSch,'NULL') || 'for school year : ' || CAST(COALESCE(schoolyear,0) AS TEXT) || 'and grade level :' || COALESCE(grade,'NULL');
		...
		error_msg = '<success>'|| 'Enrollment :' || CAST(COALESCE(newSchEnrlRecord.id,0) AS TEXT) || 'is updated for student : ' || COALESCE(newSchEnrlRecord.statestudentidentifier,'NULL') || 'in school year :' || CAST(COALESCE(schoolyear,0) AS TEXT) ;
   ...
   FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
        ...
        LOOP
          UPDATE enrollmentsrosters SET activeflag = false,modifieddate=CURRENT_TIMESTAMP, modifieduser = ceteSysAdminUserId
           ...
           RAISE NOTICE 'Student(%) with Id : % is removed from the rosterId: %  enrollmentrosterId: % ', studentsEnrollemntsRostersRecord.statestudentidentifier, studentsEnrollemntsRostersRecord.studentId,
                       studentsEnrollemntsRostersRecord.rosterId, studentsEnrollemntsRostersRecord.enrlRosterId;
           error_msg = '<success>'||'Student: '||COALESCE(studentsEnrollemntsRostersRecord.statestudentidentifier,'NULL')||' is removed from the rosterId: '||CAST(COALESCE(studentsEnrollemntsRostersRecord.rosterId,0)AS TEXT);
    ...
    FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
        ...
        LOOP
          UPDATE enrollmentsrosters SET activeflag = false,modifieddate=CURRENT_TIMESTAMP, modifieduser = ceteSysAdminUserId
           ...
           RAISE NOTICE 'Student(%) with Id : % is removed from the rosterId: %  enrollmentrosterId: % ', studentsEnrollemntsRostersRecord.statestudentidentifier, studentsEnrollemntsRostersRecord.studentId,
                       studentsEnrollemntsRostersRecord.rosterId, studentsEnrollemntsRostersRecord.enrlRosterId;
           error_msg = '<success>'||'Student: '||COALESCE(studentsEnrollemntsRostersRecord.statestudentidentifier,'NULL')||' is removed from the rosterId: '||CAST(COALESCE(studentsEnrollemntsRostersRecord.rosterId,0) AS TEXT)||'  enrollmentrosterId: '||CAST(COALESCE(studentsEnrollemntsRostersRecord.enrlRosterId,0) AS TEXT);
...
CREATE OR REPLACE FUNCTION changeEductorOnRosterWithNoCourse(att_sch_displayidentifier character varying, schoolyear bigint,
   ...
   ELSE         
           ...
           SELECT INTO ceteSysAdminUserId (SELECT id FROM aartuser WHERE username='cetesysadmin');
           SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) LIMIT 1);
           SELECT INTO old_teacherId (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE uso.organizationid = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(old_teacher_uniqueCommonId) LIMIT 1);
           SELECT INTO new_teacherId (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE uso.organizationid = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(new_teacher_uniqueCommonId) LIMIT 1);
...
CREATE OR REPLACE FUNCTION changeEductorOnRosterWithCourse(att_sch_displayidentifier character varying, schoolyear bigint,
   ...
   ELSE
           ...
           ELSE
        	...
        	UPDATE student SET esolparticipationcode = esolCode, hispanicethnicity = hispanic_Ethnicity, comprehensiverace = race, modifieddate = now(), dateofbirth = dob,
               ...
               error_msg := '<success>' || 'demographic info got changed :'|| state_student_identifier ||'not found in state :'||state_displayidentifier|| 'attendace school:'||'att_sch_displayidentifier:'||att_sch_displayidentifier||'ayp school:'||ayp_sch_displayidentifier|| 'grade:'||grade_abbrName|| 'and current school:'|| cast(coalesce(schoolyear,0) as text);
        	  ...
        	  ELSE
        		...
        		PERFORM inActivateStuTestsTrackerITITestsessions(studentTestsId := old_stuTestsRecordsInprgsPenUnusedStatus.id , inActiveStuTestSecStatusId := exitStuTestSecsStatus,
                 ...
                 ELSE
                   ...
                   error_msg := '<error>' || 'Exit withdrawal date'||  cast(coalesce(exitDate,'01/01/1900') as text) || ' is less than the school entry date '  ||  cast(coalesce(old_studentEnrollemntRecord.schoolentrydate,'01/01/1900') as text) ;
               END IF;
           END LOOP;
             END IF;
             END IF;
   ...
   ELSE
             ...
             ELSE
          	...
          	error_msg := '<success>' || coalesce(state_student_identifier,'NULL') || ':Student grade is changed to ' ;
