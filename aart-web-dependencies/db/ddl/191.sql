...
--DE6231 - split out the average linkage level from the publish_republish_test function
CREATE OR REPLACE FUNCTION calculate_avg_linkage_level(IN publishedtestid bigint)
RETURNS real AS
...
DECLARE 
	...
	BEGIN
		...
		FOREACH assessment_program_name IN ARRAY assessment_program
				LOOP
					IF ('Dynamic Learning Maps' = assessment_program_name AND ('M' = content_area OR 'ELA' = content_area)) then
...
DECLARE test_row record;
	...
	updatestudenttestmappings boolean = FALSE;
	...
	BEGIN
		...
		--added the order by unpublished.id desc LIMIT 1 so only the last unpublished test is updated
		FOR test_row IN SELECT unpublished.* FROM test published JOIN test unpublished ON published.externalid = unpublished.externalid 
 			WHERE published.id = publishedtestid AND unpublished.id != publishedtestid order by unpublished.id desc LIMIT 1
		LOOP
			...
			IF ((SELECT COUNT(*) FROM modifiedtestsections) > 0) THEN 
				...
				LOOP	 					
					...
					LOOP						
						...
						EXIT WHEN updatestudenttestmappings = TRUE;
					END LOOP;					
