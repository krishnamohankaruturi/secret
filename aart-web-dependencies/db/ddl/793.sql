...
AND otwsr.operationaltestwindowid = STUDENTTEST_ROW.operationaltestwindowid and otwsr.activeflag=true;
	...
	IF grace_period IS NOT NULL and STUDENTTEST_ROW.gracestatuscount = 0 THEN
		...
		IF (STUDENTTESTSECTION_ROW.sectionmodifiedmillis < current_millis) THEN
			IF (STUDENTTEST_ROW.assessmentprogramname = 'KAP' OR STUDENTTEST_ROW.assessmentprogramname = 'KELPA2' OR STUDENTTEST_ROW.assessmentprogramname = 'I-SMART2') THEN
	...
	IF (STUDENTTEST_ROW.assessmentprogramname = 'DLM' OR STUDENTTEST_ROW.assessmentprogramname = 'I-SMART') THEN  
		...
		FROM studentstestsections sts where sts.studentstestid = STUDENTTEST_ROW.id AND sts.statusid=inprogress_status_id)
			...
			IF ((lastmodifieddatemillis IS NULL OR current_millis_tocheck > lastmodifieddatemillis) 
				...
				--F723 Ismart Org :code to check number of sections completed
...
PERFORM (select * from autoreactivatestudentstests(in_username));
 ...
 -- cast('Practice' as character varying) AS assessmentprogramname,
  ...
  otd.schooldisplayidentifier as schooldisplayidentifier,
  st.currenttestnumber as currenttestnumber,
  st.numberoftestsrequired as numberoftestsrequired,false
  FROM studentstests st INNER JOIN test t ON st.testid = t.id
      ...
      INNER JOIN organization o ON o.id = otd.stateid
      INNER JOIN operationaltestwindow otw ON otw.id = ts.operationaltestwindowid
      ...
      AND ts.schoolyear = extract(year from o.schoolenddate)
      ORDER BY ap.programname, st.createddate, t.testname;
      ELSIF in_testtypename ilike 'AllTests'  THEN
    RETURN QUERY SELECT ap.id AS assessmentprogramId,
  ap.programname AS assessmentprogramname,
  tp.id AS testingprogramid, tp.programname AS testingprogramname,
  ca.name as contentarea,
  ...
  st.createduser, st.modifieduser,
  st.modifieddate, st.createddate,
  CAST('' as character varying) as schooldisplayidentifier,
  st.currenttestnumber as currenttestnumber,
  st.numberoftestsrequired as numberoftestsrequired,false
  FROM studentstests st INNER JOIN test t ON st.testid = t.id
      INNER JOIN student std on std.id = st.studentid
      ...
      AND (
          (otw.effectivedate <= CURRENT_TIMESTAMP AND otw.expirydate >= CURRENT_TIMESTAMP AND otw.suspendwindow IS false AND ts.suspend IS false) 
              OR (t.qccomplete IS false and otw.effectivedate IS NULL AND otw.expirydate IS NULL AND t.is_interim_test IS false)
            OR (otw.effectivedate <= CURRENT_TIMESTAMP AND ts.windowexpirydate >= CURRENT_TIMESTAMP AND t.is_interim_test IS false)
			OR (otw.effectivedate IS NULL AND ts.windowexpirydate IS NULL)
         )
  ...
  otd.schooldisplayidentifier as schooldisplayidentifier,
  st.currenttestnumber as currenttestnumber,
  st.numberoftestsrequired as numberoftestsrequired,
  (SELECT count(tcts.id) > 0 as isFieldTest
    FROM operationaltestwindowstate otws
    INNER JOIN operationaltestwindow otwsub ON otws.operationaltestwindowid = otwsub.id
    AND otwsub.expirydate > now()
    AND otwsub.assessmentprogramid = (select id from assessmentprogram where abbreviatedname = 'DLM')
    AND otwsub.testenrollmentmethodid = (SELECT tem.id FROM testenrollmentmethod tem WHERE tem.assessmentprogramid = otwsub.assessmentprogramid AND tem.methodcode = 'MLTASGNFT')
    and otwsub.activeflag is true
    INNER JOIN  operationaltestwindowstestcollections otwtc ON otws.operationaltestwindowid = otwtc.operationaltestwindowid
    INNER JOIN testcollection tcts on tcts.id = otwtc.testcollectionid AND tcts.contentareaid = ca.id AND 
    (tcts.gradecourseid IN (SELECT id from gradecourse where abbreviatedname = (select abbreviatedname from gradecourse where id = en.currentgradelevel)) OR
     tcts.gradebandid IN (
      SELECT gb.id 
      from gradeband gb 
      join gradebandgradecourse gbgc on gb.id = gbgc.gradebandid
      join gradecourse gcc on gbgc.gradecourseid = gcc.id
      where gcc.abbreviatedname = (select abbreviatedname from gradecourse where id = en.currentgradelevel)
     )
    )
    WHERE otws.stateid = std.stateid AND otws.activeflag is true
)
  FROM studentstests st INNER JOIN test t ON st.testid = t.id
      INNER JOIN student std on std.id = st.studentid
      INNER JOIN assessmentstestcollections atc ON st.testcollectionid = atc.testcollectionid
