...
DROP INDEX IF EXISTS idx_assessmentstestcollections_testcollection;
DROP INDEX IF EXISTS idx_assessmentstestcollections_assessment;
DROP INDEX IF EXISTS idx_operationaltestwindow_testcollection;
DROP INDEX IF EXISTS idx_testingprogram_assessmentprogram;
DROP INDEX IF EXISTS idx_studentstests_student;
DROP INDEX IF EXISTS idx_studentstests_testcollection;
DROP INDEX IF EXISTS idx_assessment_testingprogram;
...
CREATE INDEX idx_operationaltestwindow_testcollection ON operationaltestwindow USING btree (testcollectionid);
  ...
  LOOP
    ...
    ELSE
                ...
                WHERE st.status IN (SELECT c.id FROM category c, categorytype ct WHERE c.categorytypeid = ct.id
                                AND c.categorycode not in ('complete', 'fcunenrolled', 'fcmidunenrolled')
                                AND ct.typecode='STUDENT_TEST_STATUS')
                                AND st.activeflag = true AND otw.suspendwindow = false AND tp.programname != 'Practice'
                                AND otw.effectivedate <= CURRENT_TIMESTAMP AND otw.expirydate >= CURRENT_TIMESTAMP
