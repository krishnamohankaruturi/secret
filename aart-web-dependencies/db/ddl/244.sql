--tde json->java->json changes
DROP FUNCTION randomizestudenttest(text);
...
$BODY$
  ...
  FOR studenttestid IN (select sts.id from studentstests sts where sts.testid is null and sts.studentid = student_id and sts.activeflag is true)
  LOOP 
	...
	WHERE st.status IN (SELECT c.id FROM category c, categorytype ct WHERE c.categorytypeid = ct.id
		...
		AND s.username = in_username order by st.id)
    LOOP
	...
	IF grace_period IS NOT NULL THEN
		FOR studenttestsection IN (select id, statusid, (EXTRACT(EPOCH FROM modifieddate)+grace_period) as sectionmodifiedmillis from studentstestsections where studentstestid=studenttest.id
						AND statusid=inprogress_status_id)
		LOOP
	...
	'Practice' AS assessmentprogramname,
