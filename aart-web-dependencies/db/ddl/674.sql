...
--			update public.studentstests set status = test_timedout_status_id, modifieddate=now() where id = STUDENTTESTSECTION_ROW.studentstestid;
			...
			insert into studentstestshistory (studentstestsid, studentstestsstatusid, action, acteduser, acteddate)
			    values (STUDENTTESTSECTION_ROW.studentstestid, STUDENTTESTSECTION_ROW.id, 'REACTIVATION', 13, now());
		END IF;
...
--F607 changes
ALTER TABLE firstcontactsurveysettings ADD COLUMN mathflag boolean default true;
ALTER TABLE firstcontactsurveysettings ADD COLUMN elaflag boolean default true;
ALTER TABLE survey ADD COLUMN includeelaflag boolean default true;
ALTER TABLE survey ADD COLUMN includemathflag boolean default true;
