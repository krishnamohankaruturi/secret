...
alter table helpcontentslug  add column views bigint default 0;


CREATE OR REPLACE FUNCTION customKAPStudentTestExtract(
	in_studentids character varying[])
    RETURNS  TABLE(state text, district text, school text, schoolidentifer text, grade text, studentlastname text, studentfirstname text,
                        studentmiddlename text, statestudentidentifier text, localstudentidentifier text, enrollmentid text, enrollmentstatus text, subject text,
                        testsessionname text, testsessionid text, testid text, testcollectionid text, teststatus text, testactiveflag text, stage text, totalitems text,
                        totalomitteditems text, teststartdate text, testenddate text, specialcircumstance text, specialcircumstancestatus text,
                        lastreactivateddatetime text)
    LANGUAGE 'plpgsql'
    COST 100.0
...
TEMPORARY TABLE tmp_student_text_extract (State TEXT,District TEXT,School TEXT,SchoolIdentifier TEXT,Grade TEXT,
  ...
  FOR strecord IN ( SELECT  DISTINCT st.id AS studentstestsid, gc.abbreviatedname as grade,
	          ...
	          FROM studentstests st 
	              ...
	              JOIN student s ON st.studentid = s.id AND s.statestudentidentifier = ANY(ARRAY[ in_studentids]) AND s.stateid = 51
...
RETURN  QUERY select * from tmp_student_text_extract  ;   
              
DROP TABLE  tmp_student_text_extract;       
