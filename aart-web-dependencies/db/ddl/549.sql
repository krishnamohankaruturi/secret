...
--upgraded version from 522 to 539 to 546
...
$BODY$
 ...
 BEGIN
   error_msg :='';
   schEntryDate:=((schEntryDate::timestamp) AT TIME ZONE 'CDT');
   distEntryDate:=((distEntryDate::timestamp) AT TIME ZONE 'CDT');
   state_EntryDate:=((state_EntryDate::timestamp) AT TIME ZONE 'CDT');
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(trim(org.tree_stateDisplayidentifier))=lower(trim(stateDisplayidentifier))
...
CREATE OR REPLACE FUNCTION changeEductorOnRosterWithCourse(att_sch_displayidentifier character varying, schoolyear bigint,
   ...
   ELSE
        ...
        $BODY$
         ...
         BEGIN
            error_msg :='';
            new_schEntryDate:=((new_schEntryDate::timestamp) AT TIME ZONE 'CDT');
            new_DistEntryDate:=((new_DistEntryDate::timestamp) AT TIME ZONE 'CDT');
            exitDate:=((exitDate::timestamp) AT TIME ZONE 'CDT');
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(trim(org.tree_stateDisplayidentifier))=lower(trim(stateDisplayidentifier))
