...
$BODY$
 ...
 BEGIN
   ...
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(org.tree_stateDisplayidentifier)=lower(stateDisplayidentifier)
            and (lower(org.tree_schooldisplayidentifier)=lower(att_sch_displayidentifier) or lower(org.tree_schooldisplayidentifier)=lower(ayp_sch_displayidentifier))  
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1 
   THEN 
       error_msg := '<error> Duplicate display Identifier on school more info>> '||'Student:'||COALESCE(state_student_identifier,'NULL')||';State:'||COALESCE(stateDisplayidentifier,'NULL')||';AND Attendance school:'||COALESCE(att_sch_displayidentifier,'NULL')||';OR AYP School:'||COALESCE(ayp_sch_displayidentifier,'NULL');
       RAISE NOTICE '%',error_msg;
   ELSE  
   SELECT INTO state_Id (SELECT id FROM organization WHERE lower(displayidentifier) = lower(stateDisplayidentifier) AND activeflag is true); 
   ...
   SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) AND activeflag is true LIMIT 1);
   SELECT INTO teacher_Id (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE uso.organizationid = att_sch_id AND au.activeflag is true AND  case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(teacher_uniqueCommonId) LIMIT 1);

   IF((SELECT count(*) FROM roster r WHERE   r.activeflag is true AND lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id
   ...
   ELSE
    IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = state_student_identifier AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true) <= 0 ) THEN
        ...
        error_msg := '<error>'||'Studentid:'||COALESCE(state_student_identifier,'NULL')||'; not found with ayp:'||COALESCE(ayp_sch_displayidentifier,'NULL')||';attendance:'||COALESCE(att_sch_displayidentifier,'NULL')||';in school year:'||CAST(COALESCE(schoolyear,0) AS TEXT);
    ELSE
         FOR enrollmentRecord IN(SELECT en.* FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = state_student_identifier AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true)
       LOOP
        ...
        LOOP
           IF((SELECT count(*) FROM enrollmentsrosters WHERE activeflag is true AND enrollmentid = enrollmentRecord.id AND rosterid = rosterRecord.id) <= 0) THEN
...
$BODY$
 ...
 BEGIN
   ...
   SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) AND activeflag is true LIMIT 1);
   SELECT INTO teacher_Id (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE au.activeflag is true AND  uso.organizationid = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(teacher_uniqueCommonId) LIMIT 1);
   SELECT INTO course_Id (SELECT id FROM gradecourse WHERE lower(abbreviatedname) = lower(course_Abbrname) AND course is true AND activeflag is true LIMIT 1);
  error_msg ='';
   ...
   ELSE
    IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
        ...
        RAISE NOTICE 'Studentid % not found with ayp %, attendance %  in school year %', state_student_identifier, ayp_sch_displayidentifier, att_sch_displayidentifier, schoolyear;
        error_msg := '<error>'||'Studentid:'||COALESCE(state_student_identifier,'NULL')||';not found with ayp:'||COALESCE(ayp_sch_displayidentifier,'NULL')||';attendance:'||COALESCE(att_sch_displayidentifier,'NULL')||';in school year:'||CAST(COALESCE(schoolyear,0) AS TEXT);
    ELSE
         FOR enrollmentRecord IN(SELECT en.* FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = state_student_identifier AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true)
       LOOP
         FOR rosterRecord IN(SELECT r.* FROM roster r WHERE  r.activeflag is true AND lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id AND r.statecoursesid = course_Id
...
$BODY$
 ...
 BEGIN
   ...
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
       ...
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1
   THEN 
       error_msg := '<error> Duplicate display Identifier on school more info>> '||'Student:'||COALESCE(state_student_identifiers,'NULL')||';State:'||COALESCE(stateDisplayidentifier,'NULL')||';AND Attendance school:'||COALESCE(att_sch_displayidentifier,'NULL')||';OR AYP School:'||COALESCE(ayp_sch_displayidentifier,'NULL');
       RAISE NOTICE '%',error_msg;
   ELSE 
   SELECT INTO state_Id (SELECT id FROM organization WHERE lower(displayidentifier) = lower(stateDisplayidentifier) AND activeflag is true);
   SELECT INTO att_sch_id (SELECT schoolid FROM organizationtreedetail WHERE stateid = state_Id AND lower(schooldisplayidentifier) = lower(att_sch_displayidentifier));
  ...
  IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
        ...
        RAISE NOTICE 'Student % is not found with ayp %, attendance %  in school year %', student_identify, ayp_sch_displayidentifier, att_sch_displayidentifier, schoolyear;
        error_msg := '<error>'||'Studentid:'||COALESCE(student_identify,'NULL')||';  not found with ayp:'||COALESCE(ayp_sch_displayidentifier,'NULL')||';attendance:'||COALESCE(att_sch_displayidentifier,'NULL')||';in school year:'||CAST(COALESCE(schoolyear,0) AS TEXT);
    ELSE
   IF((SELECT count(*) FROM roster r WHERE  r.activeflag is true AND  lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id
                          AND currentschoolyear = schoolyear AND teacherid = teacher_Id)) <= 0 THEN
   ...
   ELSE
    FOR enrollmentRecord IN(SELECT stu.statestudentidentifier, en.* FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
         AND stu.statestudentidentifier = ANY(state_student_identifiers)AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true)
     LOOP
        FOR rosterRecord IN(SELECT r.* FROM roster r WHERE r.activeflag is true AND  lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id
                        AND currentschoolyear = schoolyear AND teacherid = teacher_Id)
	 LOOP
           IF((SELECT count(*) FROM enrollmentsrosters WHERE activeflag is true AND enrollmentid = enrollmentRecord.id AND rosterid = rosterRecord.id) <= 0) THEN
...
$BODY$
 ...
 BEGIN
   ...
   SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) AND activeflag is true LIMIT 1);
   SELECT INTO teacher_Id (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE au.activeflag is true AND  uso.organizationid  = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(teacher_uniqueCommonId) LIMIT 1);
   SELECT INTO course_Id (SELECT id FROM gradecourse WHERE abbreviatedname = course_abbrName and activeflag is true LIMIT 1);
   error_msg := '';
  IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = ANY(state_student_identifiers)AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true) <= 0) THEN
...
$BODY$
 ...
 BEGIN
   ...
   SELECT INTO ceteSysAdminUserId (SELECT id FROM aartuser WHERE username='cetesysadmin');
   SELECT INTO course_Id (SELECT id FROM gradecourse WHERE lower(abbreviatedname) = lower(course_Abbrname) and activeflag is true and course is true LIMIT 1);
   SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) and activeflag is true LIMIT 1);
   SELECT INTO teacher_Id (select DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE au.activeflag is true AND  uso.organizationid  = att_sch_id and case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(teacher_uniqueCommonId) LIMIT 1);
   error_msg := '';
   ...
   ELSE
    FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
           ...
           WHERE enrl.rosterid IN (SELECT r.id FROM roster r
               WHERE r.activeflag is true AND r.statecoursesid = course_Id  and r.activeflag is true and r.teacherid = teacher_id AND r.statesubjectareaid = subject_Id AND r.attendanceschoolid = att_sch_id AND currentschoolyear = schoolyear AND lower(r.coursesectionname) = lower(roster_name))
 ...
 BEGIN
   ...
   ELSE
     FOR studentsEnrollemntsRostersRecord IN (SELECT stu.statestudentidentifier, stu.id as studentId, en.id as enrollmentid, enrl.id as enrlRosterId, enrl.rosterId as rosterId
           FROM student stu JOIN enrollment en on stu.id = en.studentid JOIN enrollmentsrosters enrl on enrl.enrollmentid = en.id and enrl.activeflag is true and stu.activeflag is true and  en.activeflag is true
           WHERE enrl.rosterid IN (SELECT r.id FROM roster r 
               WHERE r.activeflag is true AND  r.teacherid = teacher_id AND   r.activeflag is true and r.statesubjectareaid = subject_Id AND r.attendanceschoolid = att_sch_id AND currentschoolyear = schoolyear AND lower(r.coursesectionname) = lower(roster_name))
...
LANGUAGE plpgsql VOLATILE
  ...
  $BODY$
   ...
   ELSE   
  	...
  	error_msg := '';
   IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = statestudent_identifier AND stu.stateid = state_Id) <= 0) THEN
 ...
 BEGIN
   ...
   ELSE
	...
	SELECT INTO ceteSysAdminUserId (SELECT id FROM aartuser WHERE username='cetesysadmin');
        ...
        SELECT INTO student_id (SELECT id FROM student WHERE lower(statestudentidentifier) = lower(statestudent_identifier) AND stateid = state_Id AND activeflag is true LIMIT 1);
    ...
    ELSE
      IF((SELECT  count(*) FROM enrollment WHERE studentid = student_id AND currentschoolyear = schoolyear and activeflag is true
  ...
  RAISE NOTICE 'Beginning remove student from roster with course';
   IF((SELECT count(stu.*)
           ...
           enrl.rosterid IN (SELECT r.id FROM roster r
               WHERE r.activeflag is true AND  r.statecoursesid = course_Id AND r.teacherid = teacher_id AND r.statesubjectareaid = subject_Id AND r.attendanceschoolid = att_sch_id AND currentschoolyear = schoolyear AND lower(r.coursesectionname) = lower(roster_name))
...
$BODY$
 ...
 BEGIN
   ...
   IF((SELECT count(1)
		   ...
		   WHERE lower(stu.statestudentidentifier) = lower(statestudent_identifier) AND
		   enrl.rosterid IN (SELECT r.id FROM roster r
		       WHERE r.activeflag is true AND  r.teacherid = teacher_id AND r.statesubjectareaid = subject_Id AND r.attendanceschoolid = att_sch_id AND currentschoolyear = schoolyear AND lower(r.coursesectionname) = lower(roster_name))
...
CREATE OR REPLACE FUNCTION changeEductorOnRosterWithNoCourse(att_sch_displayidentifier character varying, schoolyear bigint,
   ...
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(org.tree_stateDisplayidentifier)=lower(stateDisplayidentifier)
            and (lower(org.tree_schooldisplayidentifier)=lower(att_sch_displayidentifier))  
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1
   THEN 
   ...
   ELSE         
           ...
           SELECT INTO ceteSysAdminUserId (SELECT id FROM aartuser WHERE username='cetesysadmin');
           SELECT INTO subject_Id (SELECT id FROM contentarea WHERE lower(abbreviatedname)  = lower(subject_abbrName) AND activeflag is true  LIMIT 1);
           SELECT INTO old_teacherId (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE au.activeflag is true AND  uso.organizationid  = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(old_teacher_uniqueCommonId) LIMIT 1);
           SELECT INTO new_teacherId (SELECT DISTINCT au.id FROM aartuser au JOIN usersorganizations uso ON uso.aartuserid = au.id WHERE au.activeflag is true AND  uso.organizationid  = att_sch_id AND case when lower(coalesce(uniquecommonidentifier,''))='' then lower(email) else  lower(uniquecommonidentifier) end = lower(new_teacher_uniqueCommonId) LIMIT 1);
...
CREATE OR REPLACE FUNCTION changeEductorOnRosterWithCourse(att_sch_displayidentifier character varying, schoolyear bigint,
   ...
   ELSE
     ...
     ELSE
           IF((SELECT count(*) FROM roster r WHERE r.activeflag is true AND  lower(r.coursesectionname) = lower(roster_name) AND attendanceschoolid = att_sch_id AND r.statesubjectareaid = subject_Id AND r.statecoursesid = course_Id
                                  AND currentschoolyear = schoolyear AND teacherid = old_teacherId)) <= 0 THEN
        ...
        CREATE OR REPLACE FUNCTION updateStudentDemoGraphics(student_firstName character varying, student_LastName character varying, state_student_identifier character varying, state_displayidentifier character varying, hispanic_Ethnicity boolean, race character varying, esolCode character varying,
           att_sch_displayidentifier character varying, ayp_sch_displayidentifier character varying, schoolyear bigint, grade_abbrName character varying, birthDate date) RETURNS text AS
        ...
        $BODY$
         ...
         BEGIN
              error_msg :='';
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(org.tree_stateDisplayidentifier)=lower(state_displayidentifier)
            and (lower(org.tree_schooldisplayidentifier)=lower(att_sch_displayidentifier) or lower(org.tree_schooldisplayidentifier)=lower(ayp_sch_displayidentifier))  
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1
   THEN 
       ...
       RAISE NOTICE '%',error_msg;
   ELSE
           SELECT INTO state_Id (SELECT id FROM organization WHERE lower(displayidentifier) = lower(state_displayidentifier) AND activeflag is true );
           SELECT INTO att_sch_id (SELECT schoolid FROM organizationtreedetail WHERE stateid = state_Id AND lower(schooldisplayidentifier) = lower(att_sch_displayidentifier));
           SELECT INTO ayp_sch_id (SELECT schoolid FROM organizationtreedetail WHERE stateid = state_Id AND lower(schooldisplayidentifier) = lower(ayp_sch_displayidentifier));
           SELECT INTO ceteSysAdminUserId (SELECT id FROM aartuser WHERE username='cetesysadmin');
           SELECT INTO grade_Id (SELECT id FROM gradecourse gc WHERE contentareaid IS NULL AND assessmentprogramgradesid IS NOT NULL
        					AND abbreviatedname = grade_abbrName AND activeflag is true );

           IF((SELECT count(stu.*) FROM student stu JOIN enrollment en ON en.studentid = stu.id
                      WHERE stu.statestudentidentifier = state_student_identifier AND stu.stateid = state_Id  and en.activeflag is true and stu.activeflag is true AND en.attendanceschoolid = att_sch_id
           ...
           ELSE
        	...
        	UPDATE student SET esolparticipationcode = esolCode, hispanicethnicity = hispanic_Ethnicity, comprehensiverace = race, modifieddate = now(), dateofbirth = dob,
               ...
               error_msg := '<success>' || 'demographic info got changed Student:'|| state_student_identifier ||';in state :'||state_displayidentifier||';att_sch_displayidentifier:'||att_sch_displayidentifier||';ayp school:'||ayp_sch_displayidentifier||';grade:'||grade_abbrName||';and current school:'|| cast(coalesce(schoolyear,0) as text);
   ...
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(org.tree_stateDisplayidentifier)=lower(stateDisplayidentifier)
            and (lower(org.tree_schooldisplayidentifier)=lower(old_attSch) or lower(org.tree_schooldisplayidentifier)=lower(old_aypSch))  
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1
   THEN 
   ...
   IF (select count(*) from (select o.schooldisplayidentifier tree_schooldisplayidentifier,o.stateDisplayidentifier tree_stateDisplayidentifier from organizationtreedetail o) org 
            where lower(org.tree_stateDisplayidentifier)=lower(stateDisplayidentifier)
            and (lower(org.tree_schooldisplayidentifier)=lower(new_attSch) or lower(org.tree_schooldisplayidentifier)=lower(new_AypSch))  
       group by org.tree_schooldisplayidentifier,org.tree_stateDisplayidentifier order by 1 desc limit 1 ) >1
   THEN 
   ...
   ELSE
             ...
             SELECT INTO old_grade_Id (SELECT id FROM gradecourse gc WHERE contentareaid IS NULL AND assessmentprogramgradesid IS NOT NULL
          					AND abbreviatedname = old_grade AND activeflag is true );
             SELECT INTO new_grade_Id (SELECT id FROM gradecourse gc WHERE contentareaid IS NULL AND assessmentprogramgradesid IS NOT NULL
          					AND abbreviatedname = new_grade AND activeflag is true );
             SELECT INTO exitStuTestSecsStatus (SELECT c.id FROM category c INNER JOIN categorytype ct ON c.categorytypeid=ct.id WHERE ct.typecode='STUDENT_TESTSECTION_STATUS' AND categorycode = 'exitclearunenrolled');
             SELECT INTO exitStuTestsStatus (SELECT c.id FROM category c INNER JOIN categorytype ct ON c.categorytypeid=ct.id WHERE ct.typecode='STUDENT_TEST_STATUS' AND categorycode = 'exitclearunenrolled');
             ...
             IF((SELECT count(stu.*) FROM student stu JOIN enrollment en ON en.studentid = stu.id
          	...
          	RAISE NOTICE 'Student % not found in state %, attendace school %, ayp school %, grade %, and current school %', state_student_identifier,state_displayidentifier, att_sch_displayidentifier, ayp_sch_displayidentifier, old_grade, schoolyear;
          	error_msg := '<error>' || 'Student:'||coalesce(state_student_identifier,'NULL')|| ';not found in state:' ||coalesce(state_displayidentifier,'NULL')||';attendace school:'||coalesce(att_sch_displayidentifier,'NULL')||';ayp school:'||coalesce(ayp_sch_displayidentifier,'NULL')||';grade:'||coalesce(old_grade,'NULL')||';and current school:'|| cast(coalesce(schoolyear,0)as text);
                  RAISE NOTICE '%',error_msg;
             ELSE
