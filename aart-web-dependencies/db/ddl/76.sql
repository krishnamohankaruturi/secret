...
CREATE TABLE autoregistrationcriteria(
	...
	CONSTRAINT fk_autoregistrationcriteria_contentareatesttypesubjectareaid FOREIGN KEY 
(contentareatesttypesubjectareaid)
	...
	CONSTRAINT fk_autoregistrationcriteria_assessmentid FOREIGN KEY (assessmentid)
		REFERENCES assessment (id) MATCH FULL
			ON UPDATE NO ACTION ON DELETE NO ACTION,	
	CONSTRAINT uk_autoregistrationcriteria_ap_tp_asst_cattsa UNIQUE 

(assessmentprogramid,testingprogramid,assessmentid,contentareatesttypesubjectareaid)
)WITH (
  OIDS=FALSE
);
