-- Function: dlm_testlet_reset(character varying[])

-- DROP FUNCTION dlm_testlet_reset(character varying[]);

CREATE OR REPLACE FUNCTION dlm_testlet_reset(IN student_id bigint, IN testsession_id bigint, IN content_area_id bigint)
  RETURNS VOID as
  $BODY$
...
update studenttracker set status = 'UNTRACKED' where id in (select studenttrackerid from tmp_dlm_data);
...
alter table studentstestsections add column lastactive timestamp;
