...
--for CB needs to change the length of the contentcode
DROP VIEW node_report;
DROP VIEW answered_item_summary_by_node;
DROP VIEW correct_response_summary_by_node;
DROP VIEW in_correct_response_summary_by_node;
DROP VIEW total_item_summary_by_node;

ALTER TABLE contentframeworkdetail ALTER COLUMN contentcode TYPE character varying(40);
...
CREATE OR REPLACE VIEW answered_item_summary_by_node AS 
 SELECT st.id AS students_tests_id, st.studentid AS student_id, 
   ...
   FROM studentsresponses sr, taskvariantsfoils tvf, 
    taskvariantlearningmapnode tvln, taskvariantcontentframeworkdetail tvcfd, 
    contentframeworkdetail cfd, studentstests st
  WHERE sr.studentstestsid = st.id AND tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tvf.taskvariantid AND sr.taskvariantid = tvf.taskvariantid AND tvf.taskvariantid = tvln.taskvariantid AND (tvf.foilid = sr.foilid OR tvf.foilid IS NULL AND sr.response IS NOT NULL AND sr.response ~~ '[%]'::text AND btrim(btrim(sr.response, '['::text), ']'::text) ~ similar_escape('(\d+)(,\s*\d+)*'::text, NULL::text) AND (tvf.foilid = ANY (string_to_integer_array(sr.response))))
  GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode;
  
ALTER TABLE answered_item_summary_by_node
  OWNER TO aart;
...
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE answered_item_summary_by_node TO aart_user;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE answered_item_summary_by_node TO etl_user;  
...
CREATE OR REPLACE VIEW total_item_summary_by_node AS 
 SELECT st.id AS students_tests_id, st.studentid AS student_id, 
  ...
  WHERE tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tstv.taskvariantid AND st.testid = ts.testid AND tstv.testsectionid = ts.id AND st.status = ststat.id AND ststat.categorycode::text <> 'unused'::text AND metatype.id = ststat.categorytypeid AND metatype.typecode::text = 'STUDENT_TEST_STATUS'::text AND tstv.taskvariantid = tvln.taskvariantid
  GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode, cfd.description; 
...
CREATE OR REPLACE VIEW correct_response_summary_by_node AS 
         SELECT st.id AS students_tests_id, st.studentid AS student_id, 
           ...
           FROM studentsresponses sr, taskvariantsfoils tvf, 
            ...
            taskvariantcontentframeworkdetail tvcfd, contentframeworkdetail cfd, 
            studentstests st
          WHERE sr.studentstestsid = st.id AND tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tvf.taskvariantid AND sr.taskvariantid = tvf.taskvariantid AND tvf.taskvariantid = tvln.taskvariantid AND tvf.foilid = sr.foilid AND tvf.correctresponse IS NOT NULL AND tvf.correctresponse = true
          GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode
UNION ALL 
         SELECT st.id AS students_tests_id, st.studentid AS student_id, 
            st.testid AS test_id, st.testcollectionid AS test_collection_id, 
           ...
           FROM studentsresponses sr, taskvariantsfoils tvf, 
            ...
            taskvariantcontentframeworkdetail tvcfd, contentframeworkdetail cfd, 
            studentstests st
          WHERE sr.studentstestsid = st.id AND tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tvf.taskvariantid AND sr.taskvariantid = tvf.taskvariantid AND tvf.taskvariantid = tvln.taskvariantid AND tvf.foilid IS NULL AND sr.response IS NOT NULL AND sr.response ~~ '[%]'::text AND btrim(btrim(sr.response, '['::text), ']'::text) ~ similar_escape('(\d+)(,\s*\d+)*'::text, NULL::text) AND (tvf.foilid = ANY (string_to_integer_array(sr.response))) AND tvf.correctresponse IS NOT NULL AND tvf.correctresponse = true
          GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode;

ALTER TABLE correct_response_summary_by_node
  OWNER TO aart;
...
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE correct_response_summary_by_node TO aart_user;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE correct_response_summary_by_node TO etl_user;          
...
CREATE OR REPLACE VIEW in_correct_response_summary_by_node AS 
         SELECT st.id AS students_tests_id, st.studentid AS student_id, 
           ...
           FROM studentsresponses sr, taskvariantsfoils tvf, 
            ...
            taskvariantcontentframeworkdetail tvcfd, contentframeworkdetail cfd, 
            studentstests st
          WHERE sr.studentstestsid = st.id AND tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tvf.taskvariantid AND sr.taskvariantid = tvf.taskvariantid AND tvf.taskvariantid = tvln.taskvariantid AND tvf.foilid = sr.foilid AND (tvf.correctresponse IS NULL OR tvf.correctresponse = false)
          GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode
UNION ALL 
         SELECT st.id AS students_tests_id, st.studentid AS student_id, 
            st.testid AS test_id, st.testcollectionid AS test_collection_id, 
           ...
           FROM studentsresponses sr, taskvariantsfoils tvf, 
            ...
            taskvariantcontentframeworkdetail tvcfd, contentframeworkdetail cfd, 
            studentstests st
          WHERE sr.studentstestsid = st.id AND tvcfd.contentframeworkdetailid = cfd.id AND tvcfd.isprimary = true AND tvcfd.taskvariantid = tvf.taskvariantid AND sr.taskvariantid = tvf.taskvariantid AND tvf.taskvariantid = tvln.taskvariantid AND tvf.foilid IS NULL AND sr.response IS NOT NULL AND sr.response ~~ '[%]'::text AND btrim(btrim(sr.response, '['::text), ']'::text) ~ similar_escape('(\d+)(,\s*\d+)*'::text, NULL::text) AND (tvf.foilid = ANY (string_to_integer_array(sr.response))) AND (tvf.correctresponse IS NULL OR tvf.correctresponse = false)
          GROUP BY st.id, st.studentid, st.testid, tvln.nodecode, cfd.contentcode;  

ALTER TABLE in_correct_response_summary_by_node
  OWNER TO aart;
...
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE in_correct_response_summary_by_node TO aart_user;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE in_correct_response_summary_by_node TO etl_user;             
