...
ALTER TABLE studentspecialcircumstance ALTER COLUMN specialcircumstanceid DROP NOT NULL;
...
$BODY$
 ...
 BEGIN
   ...
   error_msg := '';
  IF((SELECT count(en.*) FROM enrollment en JOIN student stu ON stu.id = en.studentid AND en.currentschoolyear = schoolyear AND en.aypschoolid = ayp_sch_id AND en.attendanceschoolid = att_sch_id
           AND stu.statestudentidentifier = ANY(state_student_identifiers) AND stu.stateid = state_Id) <= 0) THEN
