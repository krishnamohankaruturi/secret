 ...
 ALTER TABLE studentreport ADD CONSTRAINT studentreport_levelid_fk FOREIGN KEY (levelid) REFERENCES leveldescription(id);

 
 DO
$BODY$
BEGIN
      IF EXISTS (SELECT 0 FROM pg_class where relname = 'idx_studentreport_levelid') THEN
            raise NOTICE 'idx_studentreport_levelid index already exists';
      else
               CREATE INDEX idx_studentreport_levelid
			ON studentreport
		USING btree
		(levelid);
      END IF;
...
$BODY$
BEGIN
      IF EXISTS (SELECT 0 FROM pg_class where relname = 'idx_reportsubscores_studentreportid') THEN
            raise NOTICE 'idx_reportsubscores_studentreportid index already exists';
      else
                CREATE INDEX idx_reportsubscores_studentreportid
  			ON reportsubscores
  			USING btree
		  (studentreportid);
      END IF;
...
$BODY$;
 ...
 DROP INDEX IF EXISTS idx_reportsmedianscore_schoolyear;
 CREATE INDEX idx_reportsmedianscore_schoolyear
  ON reportsmedianscore
  USING btree
  (schoolyear);

DROP INDEX IF EXISTS idx_subscoresdescription_assessmentprogramid;
  CREATE INDEX idx_subscoresdescription_assessmentprogramid
  ON subscoresdescription
  USING btree
  (assessmentprogramid);

  DROP INDEX IF EXISTS idx_subscoresdescription_subjectid;
  CREATE INDEX idx_subscoresdescription_subjectid
  ON subscoresdescription
  USING btree
  (subjectid);
  
  DROP INDEX IF EXISTS idx_subscoresdescription_report;
  CREATE INDEX idx_subscoresdescription_report
  ON subscoresdescription
  USING btree
  (report);
  
  DROP INDEX IF EXISTS idx_subscoresdescription_subscoredefinitionname;
  CREATE INDEX idx_subscoresdescription_subscoredefinitionname
  ON subscoresdescription
  USING btree
  (subscoredefinitionname);
  
  DROP INDEX IF EXISTS idx_subscoresdescription_subscorereportdisplayname;
  CREATE INDEX idx_subscoresdescription_subscorereportdisplayname
  ON subscoresdescription
  USING btree
  (subscorereportdisplayname);
  
  DROP INDEX IF EXISTS idx_subscoresdescription_subscoredisplaysequence;
  CREATE INDEX idx_subscoresdescription_subscoredisplaysequence
  ON subscoresdescription
  USING btree
  (subscoredisplaysequence);
  
  DROP INDEX IF EXISTS idx_subscoresdescription_schoolyear;
  CREATE INDEX idx_subscoresdescription_schoolyear
  ON subscoresdescription
  USING btree
  (schoolyear);
  
  DROP INDEX IF EXISTS idx_studentreport_status;
  CREATE INDEX idx_studentreport_status
  ON studentreport
  USING btree
  (status);
  
DROP INDEX IF EXISTS idx_assessmentprogram_programname;
CREATE INDEX idx_assessmentprogram_programname ON assessmentprogram USING btree (programname);

DROP INDEX IF EXISTS idx_organizationrelation_organizationid;
CREATE INDEX idx_organizationrelation_organizationid ON organizationrelation USING btree (organizationid);

DROP INDEX IF EXISTS idx_organization_activeflag;
CREATE INDEX idx_organization_activeflag ON organization USING btree (activeflag);

-- Function changes from TDE team
