--Script from TDE
CREATE TABLE studentsresponsescopy AS SELECT * FROM studentsresponses sr limit 0;
ALTER TABLE studentsresponsescopy ADD COLUMN deletedtime timestamp;
...
$BODY$
  ...
  LOOP
    ...
    LOOP
      ...
      FOR studenttestsection IN (select sts.id, sts.statusid, sts.studentstestid, (EXTRACT(EPOCH FROM sts.modifieddate)+(grace_period*60)) as sectionmodifiedmillis from studentstestsections sts where sts.studentstestid=studenttest.id
      ...
      FOR studenttestsection IN (SELECT sts.id, sts.statusid, (EXTRACT(EPOCH FROM sts.lastactive)+(90*60)) AS lastactivemillis FROM studentstestsections sts where sts.studentstestid = studenttest.id AND sts.statusid=inprogress_status_id)
      LOOP
        ...
        SELECT (EXTRACT(EPOCH FROM MAX(sr.modifieddate))+(90*60)) INTO lastmodifieddatemillis FROM studentsresponses AS sr WHERE sr.studentstestsectionsid = studenttestsection.id;
		  -- check if currentTime > last responseTime AND currentTime > lastActive
          IF ((lastmodifieddatemillis IS NULL OR current_millis_tocheck > lastmodifieddatemillis) AND current_millis_tocheck > studenttestsection.lastactivemillis) THEN
            -- reset the test
