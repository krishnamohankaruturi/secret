...
-- this originally wasn't in 777.sql, but there were too many branches with DDL changes, so we can just run it manually
-- woo idempotency!
DO
$BODY$
BEGIN
    IF NOT EXISTS (
        ...
        AND table_name = 'scoringapireprocess'
    ) THEN
        RAISE NOTICE 'Did not find scoringapireprocess table or associated indexes, creating...';
        
        DROP INDEX IF EXISTS scoringapireprocess_studentid;
        DROP INDEX IF EXISTS scoringapireprocess_contentareaid;
        DROP INDEX IF EXISTS scoringapireprocess_reprocesseddate;
        DROP TABLE IF EXISTS scoringapireprocess;
        
        CREATE TABLE scoringapireprocess (
            id BIGSERIAL NOT NULL,
            studentid BIGINT NOT NULL,
            contentareaid BIGINT NOT NULL,
            reprocesseddate TIMESTAMP WITH TIME ZONE,
            CONSTRAINT scoringapireprocess_pkey PRIMARY KEY (id),
            CONSTRAINT scoringapireprocess_studentid_fkey FOREIGN KEY (studentid)
                REFERENCES student (id) MATCH SIMPLE
                ON UPDATE NO ACTION
                ON DELETE NO ACTION,
            CONSTRAINT scoringapireprocess_contentareaid_fkey FOREIGN KEY (contentareaid)
                REFERENCES contentarea (id) MATCH SIMPLE
                ON UPDATE NO ACTION
                ON DELETE NO ACTION
        );
        
        CREATE INDEX scoringapireprocess_studentid ON scoringapireprocess USING btree (studentid);
        CREATE INDEX scoringapireprocess_contentareaid ON scoringapireprocess USING btree (contentareaid);
        CREATE INDEX scoringapireprocess_reprocesseddate ON scoringapireprocess USING btree (reprocesseddate);
    ELSE
        RAISE NOTICE 'Found scoringapireprocess table, skipping creation of table and indexes';
    END IF;
END;
$BODY$;
