...
WITH RECURSIVE organization_relation(organizationid, parentorganizationid) AS (
        ...
        SELECT id, organizationname, displayidentifier, organizationtypeid, welcomemessage, buildinguniqueness, schoolstartdate, schoolenddate, contractingorganization FROM organization WHERE id IN (SELECT parentorganizationid FROM organization_relation);
