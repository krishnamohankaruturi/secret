...
CREATE OR REPLACE FUNCTION update_breached_test(_statestudentidentifiers text[], _grade text, _contentarea text, _assessmentprogram text, _stage text,_state text, _breachedtestsession text, _breachedtest bigint, _clearscope text, _orgname text,  _currentschoolyear bigint) RETURNS void AS
$BODY$
DECLARE
	stdtestid bigint;
	stdtestids bigint[];
...
BEGIN
	IF(_breachedtestsession = NULL OR _breachedtest = NULL) THEN
		RAISE EXCEPTION 'Please provide breached testsession and test form';
	END IF;
	IF (_grade is NULL OR _contentarea is NULL OR _assessmentprogram is NULL OR _stage is NULL OR _state is NULL OR _currentschoolyear is NULL ) THEN
		RAISE EXCEPTION 'Please provide all the required input variables - ContenArea, AssessmentProgram, Stage, State and Current School Year';
	END IF;
	...
	IF (breachtestsession_id IS NULL) THEN
		RAISE EXCEPTION 'No testsession found with the given information';
	END IF;
	IF (_statestudentidentifiers IS NOT NULL AND stcount > 0) THEN
	...
	ELSE
		IF (_clearscope is NULL OR _orgname is NULL) THEN
			RAISE EXCEPTION 'Please provide the scope or Studentidentifiers to clear breached tests ';
		END IF;
...
alter table operationaltestwindowstudent
