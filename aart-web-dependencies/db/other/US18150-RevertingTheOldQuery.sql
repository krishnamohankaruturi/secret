...
BEGIN
    ...
    SELECT id FROM assessmentprogram WHERE abbreviatedname = 'CPASS' INTO dlmassessmentprogram_id;
    ...
    FOR gradesneedto_removerecords IN (SELECT id, abbreviatedname FROM gradecourse WHERE abbreviatedname in ('3','4','6', '7','9','10','12') AND assessmentprogramgradesid IS not null) LOOP
        ...
        FOR enrollments_rosters_record IN (SELECT enrl.rosterid, enrl.enrollmentid, enrl.id as enrollmentsrosterid FROM student stu JOIN enrollment en ON en.studentid = stu.id
          ...
          IF((SELECT count(st.*) FROM testsession ts JOIN studentstests st ON st.testsessionid = ts.id WHERE st.enrollmentid = enrollments_rosters_record.enrollmentid AND ts.rosterid = enrollments_rosters_record.rosterid) > 0) THEN
             ...
             WITH inactivestudentresponses AS (UPDATE studentsresponses SET activeflag = TRUE, modifieddate = now(), modifieduser = (SELECT id FROM aartuser WHERE username = 'cetesysadmin')
                WHERE studentstestsid IN (SELECT st.id FROM testsession ts JOIN studentstests st ON st.testsessionid = ts.id 
                     WHERE st.enrollmentid = enrollments_rosters_record.enrollmentid AND ts.rosterid = enrollments_rosters_record.rosterid AND st.modifieddate >= '2016-04-05') RETURNING 1)
             ...
             WITH inactivestudentstestsections AS (UPDATE studentstestsections SET activeflag = TRUE, modifieddate = now(), modifieduser = (SELECT id FROM aartuser WHERE username = 'cetesysadmin')
                           WHERE studentstestid IN (SELECT st.id FROM testsession ts JOIN studentstests st ON st.testsessionid = ts.id 
                              WHERE st.enrollmentid = enrollments_rosters_record.enrollmentid AND ts.rosterid = enrollments_rosters_record.rosterid 
                              AND st.modifieddate >= '2016-04-05' and st.activeflag is false) RETURNING 1)
             ...
             WITH inactivestudentstests AS (UPDATE studentstests SET activeflag = TRUE, modifieddate = now(), modifieduser = (SELECT id FROM aartuser WHERE username = 'cetesysadmin')
             ...
             WITH inactiveenrollmentsrosters AS (UPDATE enrollmentsrosters SET activeflag = TRUE, modifieddate = now(), modifieduser = (SELECT id FROM aartuser WHERE username = 'cetesysadmin')
                                       WHERE id = enrollments_rosters_record.enrollmentsrosterid AND modifieddate >= '2016-04-05' and activeflag is false RETURNING 1) SELECT count(*) FROM inactiveenrollmentsrosters INTO updatecount;                                       
             ...
             IF((SELECT count(*) FROM studentstests WHERE testsessionid IN (SELECT id FROM testsession WHERE rosterid = enrollments_rosters_record.rosterid)) <= 1) THEN
                  ...
                  WITh inactivetestsessions AS (UPDATE testsession SET activeflag = TRUE, modifieddate = now(), modifieduser = (SELECT id FROM aartuser WHERE username = 'cetesysadmin')
                       WHERE rosterid =  enrollments_rosters_record.rosterid AND modifieddate >= '2016-04-05' and activeflag is false RETURNING 1) SELECT count(*) FROM inactivetestsessions INTo updatecount;
        ...
        msg := msg || ' Total studentsresponses activated = ' || totalStuentresponsesinactivated || ', Total studentstestsections activated = ' || totalstudenttestsectionsinactivated
               || ', Total studentstests activated = ' || totalstudentstestsinactivated || ', Total enrollmentsrosters activated = ' || totalenrollmentrostersinactivated
               || ', Total testsessions activated = ' || totaltestsessionsinactivated;
