\COPY (
	select s.id as "Student DB ID",
		...
		rss.subscorerawscore as "Subscore Raw Score", rss.rating as "Rating"
	from student s
	...
	order by ca.abbreviatedname, otd.districtname, otd.schoolname, gc.abbreviatedname::integer, s.statestudentidentifier, rss.subscoredefinitionname
) TO 'ela-math-subscores.csv' DELIMITER ',' CSV HEADER;