...
def extract_students(options, db, sqliteconn):
    ...
    if options.assessment == '3':
		cursor1.execute("""
				/*NO LOAD BALANCE*/
		  WITH nonqc_schools as ( select id schoolid from organization where id not in (select id from organization where  organizationtypeid=2 and 
          organizationname in ('AMP QC State','ARMM','ARMM QC State','ATEA','ATEA QC State','cPass QC State','Demo State','DLM QC EOY State','DLM QC IM State ','DLM QC State','DLM QC YE State'
          ,'Flatland','KAP QC State','MY ORGANIZATION ','NY Training State','PII Deactivation','Playground QC State','QA QC State','Service Desk QC State','Training Program'))
					and organizationtypeid=7)				
				SELECT
			...
			e.currentschoolyear, s.firstlanguage,
					...
					cwrt.categorydescription  as "Wrt band"
				FROM student s
				JOIN enrollment e ON (s.id = e.studentid)
				inner join nonqc_schools org on org.schoolid=e.attendanceschoolid
				...
				LEFT JOIN category cwrt  ON s.writingbandid=cwrt.id
		...
		for row in cursor1:
			...
			sqlitecur.execute("""
				INSERT INTO student (id, activeflag, enrollmentid, legalfirstname,
					...
					activeenrollmentflag, exitwithdrawaldate, exitwithdrawaltype,sci_band,final_sci,writing_band
					)
				VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
			""",
							  [
		...
		cursor1.execute("""
				/*NO LOAD BALANCE*/				
		  WITH nonqc_schools as ( select schoolid  from organizationtreedetail where stateid not in (select id from organization where  organizationtypeid=2 and 
			  organizationname in ('AMP QC State','ARMM','ARMM QC State','ATEA','ATEA QC State','cPass QC State','Demo State','DLM QC EOY State','DLM QC IM State ','DLM QC State','DLM QC YE State'
			  ,'Flatland','KAP QC State','MY ORGANIZATION ','NY Training State','PII Deactivation','Playground QC State','QA QC State','Service Desk QC State','Training Program')))
				SELECT
					s.id, s.activeflag, e.id, s.legalfirstname,
...
def extract_student_tests(options, db, sqliteconn):
 ...
 ,'Flatland','KAP QC State','MY ORGANIZATION ','NY Training State','PII Deactivation','Playground QC State','QA QC State','Service Desk QC State','Training Program')))					
            ...
            """
                   % (options.assessment, limiter,options.year) )
        ...
        cursor1.execute("""
            ...
            SELECT
                ...
                sts.id as studentstestssectionsid,
                sts.testsectionid as testsectionid,
                case when stcat.categorycode ='exitclearunenrolled-complete' then 'complete' else  stcat.categorycode end categorycode,
                case when stscat.categorycode ='exitclearunenrolled-complete' then 'complete' else  stscat.categorycode end categorycode,
        		st.createddate, st.startdatetime, st.enddatetime,
        		...
        		sc.cedscode as  specialcircumstancecedscode,
                sc.activeflag as specialcircumstanceactive,
				ts.operationaltestwindowid as windowid,
				ts.source,
            ...
            when ts.source='MANUAL' and tc.name ~* 'Offline' then 'Year end'
	        ...
	        LEFT JOIN specialcircumstance sc ON (stsc.specialcircumstanceid = sc.id)
                AND e.currentschoolyear=%s
            WHERE ap.id=%s %s AND e.currentschoolyear=%s AND st.activeflag=case when st.status=679 then st.activeflag else 't' end;
        """
...
def extract_student_tests_inactive(options, db, sqliteconn):
    """
    Extract all student tests
    """
    logging.debug("extract_studentstests_inactive started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '3':
        if not _is_table(sqliteconn, 'studentstests_inactive'):
            sqlitecur.execute("""
                CREATE Table studentstests_inactive (
                id integer,
                studentid integer,
                enrollmentid integer,
                testid integer,
        ...
        cursor1.execute("""
	        ...
	        LEFT JOIN specialcircumstance sc ON (stsc.specialcircumstanceid = sc.id)
                AND e.currentschoolyear=%s
            WHERE ap.id=%s %s AND e.currentschoolyear=%s AND st.activeflag='f' and stcat.categorycode ~* 'complete';
        """
                    % (options.year, options.assessment, limiter,
                       options.year))
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                 INSERT INTO studentstests_inactive (id, studentid, enrollmentid, testid,
                studentstestssectionsid, testsectionid, teststatus,
                testsectionstatus, createddate, startdatetime, enddatetime,
                educatorid, finalband, programname, paperpencil,rosterid,contentareaname,
                specialcircumstanceid,
    ...
    if (options.assessment == "4"):
        ...
        cursor1.execute("""
            ...
            SELECT
                ...
                sres1.createddate,sres1.modifieddate,readableresponse::character varying(300) readableresponse
            FROM studentstests st
        ...
        if type(row["responseorder"]) is int: 
            ...
            sqlitecur.execute("""
                INSERT INTO studentsresponses (
                    ...
                    foildid, foiltext, responsetext, score, sortorder,studentsresponsesorder,createddate,modifieddate,readableresponse)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1+?,?,?,?)""", row)
        else:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
...
def extract_test(options, db, sqliteconn):
    ...
    cursor1.execute("""
		/*NO LOAD BALANCE*/
            ...
            LEFT JOIN accessibilityfile acf on acf.id = bac.accessibilityfileid AND bac.brailleabbreviation in ('UCB','UEB','EBAE')
            WHERE ap.id=%s
        """
        % options.assessment)
...
def extract_taskvariant(options, db, sqliteconn):
    """
    Extract all extract_taskvariant
    """
    ...
    if options.assessment == '3':
	    ...
	    ft.name as frameworktype, ca.abbreviatedname,
            cfd.contentcode, tvcfd.isprimary,
            ct.name, ctd.name, tlf.formatname, tvcat.categoryname, nodecode,
            tjson.numberofpossiblescores_count ,
			tjson.numberofpossiblescores ,
			tjson.max_score ,
			tjson.scoring_type 
        FROM test t
	    JOIN testsection as ts ON (t.id = ts.testid)
	    ...
	    LEFT JOIN tasksubtype tst ON tv.tasksubtypeid = tst.id
            ...
            JOIN assessmentprogram ap ON tp.assessmentprogramid = ap.id
	    JOIN contentframework cf ON (cfd.contentframeworkid = cf.id AND cf.assessmentprogramid = ap.id)
		LEFT OUTER JOIN taskvariantpossiblescoresjson tjson on tjson.externaltaskvariantid=tv.externalid
        WHERE ap.id=%s
        """
                    % options.assessment)
        rows_inserted=0
        for row in cursor1:
            ...
            sqlitecur.execute("""
				...
				contentframeworkdetailcode, primarycontentframeworkdetail,
                cognitivetaxonomyname, cognitivecategoryname,
                tasklayoutformatname, essentialelementlinkage, nodecode,numberofpossiblescores_count ,
			    numberofpossiblescores ,max_score ,scoring_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                              row)        
...
def extract_testcollections(options, db, sqliteconn):
    ...
    cursor1.execute("""
        ...
        SELECT tc.id, tc.name, tc.randomizationtype, tc.createdate,
            otc.effectivedate, otc.expirydate, g.name, gb.name,tc.pooltype
    ...
    for row in cursor1:
        ...
        sqlitecur.execute("""
            INSERT INTO testcollection (id, name, randomizationtype,
                createdate, effectivedate, expirydate, gradecourse, gradeband,pooltype)
            VALUES (?,?,?,?,?,?,?,?,?)""", row)
...
def extract_reportsubscores(options, db, sqliteconn):
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
        ...
        cursor1.execute("""/*NO LOAD BALANCE*/
				        ...
				        from reportsubscores r 
						WHERE exists (select 1 from studentreport sr 
				                        WHERE sr.id=r.studentreportid and sr.schoolyear = %s AND sr.assessmentprogramid = %s)
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
        ...
        if (not _is_table(sqliteconn, 'studentreport')):
            ...
            CREATE Table studentreport (id integer,
										 ...
										 generated                          boolean,
										 stage1hassccode boolean,
										 stage2hassccode boolean,
										 reading_rawscore numeric(6,3),
										 listening_rawscore numeric(6,3),
										 speaking_rawscore numeric(6,3),
										 writing_rawscore numeric(6,3),
										 reading_scalescore bigint,
										 listening_scalescore bigint,
										 speaking_scalescore bigint,
										 writing_scalescore bigint,
										 reading_standarderror numeric(6,3),
										 listening_standarderror numeric(6,3),
										 speaking_standarderror numeric(6,3),
										 writing_standarderror numeric(6,3),
										 reading_sccode boolean,
										 listening_sccode boolean,
										 speaking_sccode boolean,
										 writing_sccode boolean,
										 reading_level bigint,
										 listening_level bigint,
										 speaking_level bigint,
										 writing_level bigint,
										 overall_level bigint,
										 comprehension_scalescore bigint,
										 comprehension_rawscore numeric(6,3),
										 comprehension_standarderror numeric(6,3),
										 schoolname text,
										 districtname text)
            							 """)
...
def extract_organizationreportdetails(options, db, sqliteconn):
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
        ...
        summaryreportpath,summaryreportcsvpath,reporttype, teacherid, schoolname, detailedreportpath_csv, modifieddate, 
        createduser, modifieduser, districtname
        ...
        createddate,detailedreportpath,schoolreportpdfpath,schoolreportpdfsize,schoolreportzipsize,batchreportprocessid,gradecourseabbrname,
        summaryreportpath,summaryreportcsvpath,reporttype, teacherid, schoolname, detailedreportpath_csv, modifieddate, 
        createduser, modifieduser, districtname) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                              row)
    else:
        return
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
        ...
        if (not _is_table(sqliteconn, 'rawtoscalescores')):
            ...
            CREATE Table rawtoscalescores(id integer,schoolyear bigint,assessmentprogramid bigint,subjectid bigint,gradeid bigint,
                                          ...
                                          performance_item_weight numeric,domain text,testingprogramid bigint,reportcycle text)
                              """)
        ...
        performance_rawscore_include_flag,performance_item_weight,domain,testingprogramid,reportcycle
        ...
        performance_rawscore_include_flag,performance_item_weight,domain,testingprogramid,reportcycle) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                              row)
...
def extract_leveldescription(options, db, sqliteconn):
    ...
    if options.assessment == '12' or options.assessment == '37' or options.assessment == '47':
        ...
        if (not _is_table(sqliteconn, 'leveldescription')):
            sqlitecur.execute(
                "CREATE Table leveldescription(id integer,schoolyear bigint,assessmentprogramid bigint,subjectid bigint,gradeid bigint,testid1 bigint, testid2 bigint,level bigint,levelname text,batchuploadid bigint,createddate timestamp,createduser bigint,activeflag boolean,leveldescription text,descriptiontype type,descriptionparagraphpagebottom type,testingprogramid BIGINT,reportcycle text)")

        cursor1 = db.cursor()

        cursor1.execute("""/*NO LOAD BALANCE*/
            select id,schoolyear,assessmentprogramid,subjectid,gradeid,
            level,levelname,batchuploadid,createddate,createduser,activeflag,leveldescription,descriptiontype,descriptionparagraphpagebottom,testingprogramid,reportcycle
    ...
    if options.assessment == '37' or options.assessment == '47':
        ...
        cursor1.execute("""/*NO LOAD BALANCE*/
				with dup as 
						...
						,stg.code stagecode
						,ccq.source
						,scs.kelpascoringstatus status
						,c.categorycode
						,ccq.scorerid ccqscoreid
						,ccq.taskvariantid
						,ccq.nonscorereason nonscoringreason
						,ccq.score
						,ccq.createddate
						,row_number() over (partition by scs.studentid,scs.studentstestsid,ccq.taskvariantid order by ccq.createddate desc) row_num 
						...
						inner join studentstestscore ccq on ccq.studenttestid=scs.studentstestsid and ccq.activeflag is true
						...
						left outer join category c on c.id=scs.kelpascoringstatus and c.activeflag is true
        ...
        for row in cursor1:
            ...
            testsessionid, stagecode, source, status, categorycode, 
...
def extract_scoringassignmentstudent(options, db, sqliteconn):
    """
    Extract scoringassignmentstudent
    """
    ...
    if options.assessment == '37' or options.assessment == '47':
        logging.debug("extract_scoringassignmentstudent started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'scoringassignmentstudent')):
           sqlitecur.execute(
        ...
        for row in cursor1:
            ...
            id, studentstestsid, studentid, scoringassignmentid, createddate, 
            createduser, activeflag, kelpascoringstatus)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_scoringassignment(options, db, sqliteconn):
    """
    Extract scoringassignment
    """
    ...
    if options.assessment == '37' or options.assessment == '47':
        logging.debug("extract_scoringassignment started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'scoringassignment')):
           sqlitecur.execute(
        ...
        cursor1.execute("""/*NO LOAD BALANCE*/
				       select sc.id                   
							 ...
							 ,sc.rosterid  
							 ,sc.source  
							from scoringassignment sc 
							inner join testsession ts on ts.id = sc.testsessionid and ts.activeflag is true and sc.activeflag is true
							inner join testcollection tc on tc.id = ts.testcollectionid and tc.activeflag is true
							inner join assessmentstestcollections asmnttc on asmnttc.testcollectionid = ts.testcollectionid and asmnttc.activeflag is true
							inner join assessment asmnt on asmnt.id = asmnttc.assessmentid and asmnt.activeflag is true
							inner join testingprogram tp on tp.id = asmnt.testingprogramid and tp.activeflag is true
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
            INSERT INTO scoringassignment(id, testsessionid, createddate, createduser, activeflag, ccqtestname,rosterid, source)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_scoringassignmentscorer(options, db, sqliteconn):
    """
    Extract scoringassignmentscorer
    """
    ...
    if options.assessment == '37' or options.assessment == '47':
        logging.debug("extract_scoringassignmentscorer started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'scoringassignmentscorer')):
           sqlitecur.execute(
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
            INSERT INTO scoringassignmentscorer(
            id, scoringassignmentid, scorerid, createddate, createduser, activeflag)
            VALUES (?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_studentstestscore(options, db, sqliteconn):
    """
    Extract studentstestscore
    """
    ...
    if options.assessment == '37' or options.assessment == '47' or options.assessment == '12':
        logging.debug("extract_studentstestscore started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'studentstestscore')):
           sqlitecur.execute(
        ...
        for row in cursor1:
            ...
            id, studenttestid, taskvariantid, scorerid, score, nonscorereason, 
            rubriccategoryid, activeflag, source, createddate, createduser,modifieddate, modifieduser)
            VALUES (?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_ccqscore(options, db, sqliteconn):
    """
    Extract ccqscore
    """
    ...
    if options.assessment == '37' or options.assessment == '47':
        logging.debug("extract_ccqscore started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'ccqscore')):
           sqlitecur.execute(
            """CREATE TABLE ccqscore
							( id bigint,
							  scoringassignmentstudentid bigint,
							  scoringassignmentscorerid bigint,
							  isscored boolean,
							  totalscore numeric,
							  createddate timestamp,
							  createduser integer ,
        ...
        cursor1.execute("""/*NO LOAD BALANCE*/
				  select distinct 
						  ccq.id                         
						 ,ccq.scoringassignmentstudentid 
						 ,ccq.scoringassignmentscorerid  
						 ,ccq.isscored                   
						 ,ccq.totalscore                 
						 ,ccq.createddate                
						 ,ccq.createduser                
        ...
        for row in cursor1:
            ...
            id, scoringassignmentstudentid, scoringassignmentscorerid, isscored, 
            totalscore, createddate, createduser, activeflag, excludeflag, 
            notificationflag, status, source, modifieduser, modifieddate)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_ccqscoreitem(options, db, sqliteconn):
    """
    Extract ccqscoreitem
    """
    ...
    if options.assessment == '37' or options.assessment == '47':
        logging.debug("extract_ccqscoreitem started for assessmentprogram:%s" % options.assessment)
        if (not _is_table(sqliteconn, 'ccqscoreitem')):
           sqlitecur.execute(
            """CREATE TABLE ccqscoreitem
							( id bigint,
							  ...
							  activeflag boolean ,
							  taskvariantid bigint,
							  nonscoringreason bigint,
							  externalscorer bigint,
							  source text,
							  modifieduser integer,
							  modifieddate timestamp)""")
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
            INSERT INTO ccqscoreitem(
            id, ccqscoreid, rubriccategoryid, score, createddate, createduser, 
            activeflag, taskvariantid, nonscoringreason, externalscorer, 
            source, modifieduser, modifieddate)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", row)
    else:
        return
...
def extract_contentarea(options, db, sqliteconn):
    ...
    logging.debug("rows inserted:%s" %rows_inserted)
    sqliteconn.commit()
def extract_student_tests_interim(options, db, sqliteconn):
    """
    Extract all student tests interim
    """
    logging.debug("extract_studentstests_interim started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '12':
        ...
        if not _is_table(sqliteconn, 'studentstests_interim'):
            sqlitecur.execute("""
            CREATE Table studentstests_interim (
                id integer,
                studentid integer,
                enrollmentid integer,
                testid integer,
    ...
    # the case statment is to isolate paper and pencil test.
        ...
        cursor1.execute("""
            ...
            SELECT
                ...
                sc.activeflag as specialcircumstanceactive,
                st.totalrawscore,
                ct.categorycode  testspecification              
            FROM studentstests st
            JOIN test t ON st.testid = t.id
			JOIN testspecstatementofpurpose tstop on t.testspecificationid=tstop.testspecificationid
	        JOIN category ct on ct.id=tstop.statementofpurposeid 
            JOIN studentstestsections sts ON sts.studentstestid = st.id
            JOIN testcollectionstests tct ON st.testid = tct.testid
            JOIN testcollection tc ON tc.id = tct.testcollectionid
            JOIN contentarea ca ON ca.id = tc.contentareaid
            ...
            WHERE ap.id=%s AND tp.programname ='Interim' AND e.currentschoolyear=%s AND st.activeflag='t'
			    and (ct.categorycode='INSTRCT' or ct.categorycode='PRDCTN');
            """
                    % (options.year, options.assessment, 
                       options.year))
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                INSERT INTO studentstests_interim (id, studentid, enrollmentid, testid,
                        studentstestssectionsid, testsectionid, teststatus,
                        testsectionstatus, createddate, startdatetime, enddatetime,
                        educatorid, finalband, programname, paperpencil,rosterid,contentareaname,interimtheta,osbrowser,specialcircumstanceid,
		                      specialcircumstancetype,
		                            specialcircumstancecedscode,
                                    specialcircumstanceactive, totalrawscore,testspecification )
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        ...
                        row['specialcircumstanceactive'],row['totalrawscore'],row['testspecification']))
        ...
        return		
    cursor1.close()
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx1_studentstests_interim on studentstests_interim(testsectionid,teststatus,programname,createddate,startdatetime,enddatetime)");
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx2_studentstests_interim on studentstests_interim(studentid,enrollmentid,programname,teststatus)");
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx3_studentstests_interim on studentstests_interim(testid)");
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx4_studentstests_interim on studentstests_interim(studentstestssectionsid)");
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx5_studentstests_interim on studentstests_interim(studentid)");
    sqlitecur.execute("CREATE INDEX IF NOT EXISTS idx6_studentstests_interim on studentstests_interim(rosterid)");
    logging.debug("rows inserted:%s" %rows_inserted)
    sqliteconn.commit()

def extract_student_responses_interim(options, db, sqliteconn):
    """
    Extract all student responses interim
    """
    logging.debug("extract_studentsresponses_interim started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    if (options.assessment == "12"):
      if (not _is_table(sqliteconn, 'studentsresponses_interim')):
        sqlitecur.execute("""
            CREATE Table studentsresponses_interim (
                studentstestsid integer,
                studentstestsectionsid integer,
                taskvariantid integer,
                foildid integer,
    ...
    sres1.createddate,sres1.modifieddate,readableresponse::character varying(300) readableresponse
  	FROM studentstests st
        ...
        LEFT JOIN taskvariantsfoils tvf ON sres1.foilid = tvf.foilid
        LEFT JOIN taskvariant tv ON tv.id = tvf.taskvariantid
         WHERE ap.id = %s AND e.currentschoolyear=%s AND tp.programname ='Interim' AND sres1.activeflag='t'
		 and (ct.categorycode='INSTRCT' or ct.categorycode='PRDCTN')
        """ % (options.assessment, options.year))
      ...
      for row in cursor1:
          if type(row["responseorder"]) is int:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                INSERT INTO studentsresponses_interim (
                    studentstestsid, studentstestsectionsid, taskvariantid,
                    foildid, foiltext, responsetext, score, sortorder,studentsresponsesorder,createddate,modifieddate,readableresponse)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1+?,?,?,?)""", row)
          else:
...
def extract_tests_interim(options, db, sqliteconn):
    """
    Extract all tests interim
    """
    logging.debug("extract_tests_interim started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '12':
        ...
        if not _is_table(sqliteconn, 'interimtest'):
            sqlitecur.execute("""
            CREATE Table interimtest (
                interimtestid bigint,
                interimtestname text,
                interimtestgrade text,
                interimtestsubject text,
                testid bigint,
                testname text,
                externalid bigint,
		        testgrade text,
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                INSERT INTO interimtest(interimtestid, interimtestname, interimtestgrade, interimtestsubject, 
            testid, testname, externalid, testgrade, testsubject, testspecification)
                VALUES (?,?,?,?,?,?,?,?,?,?)""",row)
...
def extract_interimstudentreport(options, db, sqliteconn):
    """
    Extract all interimstudentreport
    """
    logging.debug("extract_interimstudentreport started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '12':
        ...
        if not _is_table(sqliteconn, 'interimstudentreport'):
            sqlitecur.execute("""
        ...
        cursor1.execute("""
            SELECT id, studentid, enrollmentid, gradeid, contentareaid, attendanceschoolid, 
       schoolname, districtid, districtname, stateid, schoolyear, assessmentprogramid, 
       testingprogramid, reportcycle, testid, externaltestid, studentsteststatus, 
       rawscore, scalescore, standarderror, status, scorerangedisplayreasonid, 
       exitstatus, transferred, totalincludeditemcount, respondeditemcount, 
       excludeditemcount, filepath, generated, batchreportprocessid, 
       operationaltestwindowid, createduser, modifieduser, createddate,modifieddate
	  ...
	  where assessmentprogramid=%s and schoolyear=%s
        ...
        for row in cursor1:
            ...
            id, studentid, enrollmentid, gradeid, contentareaid, attendanceschoolid, 
            schoolname, districtid, districtname, stateid, schoolyear, assessmentprogramid, 
            testingprogramid, reportcycle, testid, externaltestid, studentsteststatus, 
            rawscore, scalescore, standarderror, status, scorerangedisplayreasonid, 
            exitstatus, transferred, totalincludeditemcount, respondeditemcount, 
            excludeditemcount, filepath, generated, batchreportprocessid, 
            operationaltestwindowid, createduser, modifieduser, createddate, 
            modifieddate)
           VALUES (?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, 
            ?, ?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?, ?, ?, ?,?)""",row)
...
def extract_studentreportquestioninfo(options, db, sqliteconn):
    """
    Extract all studentreportquestioninfo
    """
    logging.debug("extract_studentreportquestioninfo started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '12':
        ...
        if not _is_table(sqliteconn, 'studentreportquestioninfo'):
            sqlitecur.execute("""
        ...
        cursor1.execute("""
            SELECT sr.id, sr.interimstudentreportid, sr.testid, sr.externaltestid, sr.taskvariantid, 
				   sr.taskvariantexternalid, sr.taskvariantposition, sr.questioninformationid, 
				   sr.creditearned, sr.createduser, sr.modifieduser, sr.createddate, sr.modifieddate
			  FROM studentreportquestioninfo sr 
			  inner join interimstudentreport istd on sr.interimstudentreportid=istd.id
			  where istd.assessmentprogramid=%s and istd.schoolyear=%s
        """ % (options.assessment, options.year))
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                INSERT INTO studentreportquestioninfo(
            id, interimstudentreportid, testid, externaltestid, taskvariantid, 
            taskvariantexternalid, taskvariantposition, questioninformationid, 
            creditearned, createduser, modifieduser, createddate, modifieddate)
			VALUES (?, ?, ?, ?, ?,?, ?, ?,?, ?, ?, ?, ?);""",row)
...
def extract_enrollmentcontentarea(options, db, sqliteconn):
    """
    Extract all enrollmentcontentarea
    """
    logging.debug("extract_enrollmentcontentarea started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '3':
        ...
        if not _is_table(sqliteconn, 'enrollmentcontentarea'):
            sqlitecur.execute("""
          CREATE TABLE enrollmentcontentarea(
					  studentid bigint,
					  statestudentidentifier text,
					  statename  text,
					  schooldisplayidentifier text,
					  districtdisplayidentifier text,
					  educatorid bigint,
					  rostercontentarea text,
					  enrollmentflag boolean,
					  enrollmentrosterflag boolean 
					  );
            """)
        ...
        cursor1.execute("""
                       select distinct s.id studentid ,s.statestudentidentifier ,ort.statename,ort.schooldisplayidentifier,ort.districtdisplayidentifier                 
                       ,aart.id educatorid ,ca.abbreviatedname rostercontentarea,e.activeflag enrollmentflag,er.activeflag enrollmentrosterflag
                       from student s 
                       ...
                       where s.activeflag is true and assessmentprogramid=%s and e.currentschoolyear=%s
        ...
        for row in cursor1:
            rows_inserted=rows_inserted+1
            sqlitecur.execute("""
                INSERT INTO enrollmentcontentarea(
            studentid, statestudentidentifier, statename, schooldisplayidentifier, 
            districtdisplayidentifier, educatorid, rostercontentarea, enrollmentflag, 
            enrollmentrosterflag)
            VALUES (?, ?, ?, ?,  ?, ?, ?, ?, ?);""",row)
...
def extract_questioninformation(options, db, sqliteconn):
    """
    Extract all questioninformation
    """
    logging.debug("extract_questioninformation started for assessmentprogram:%s" % options.assessment)
    sqlitecur = sqliteconn.cursor()
    ...
    if options.assessment == '12':
        ...
        if not _is_table(sqliteconn, 'questioninformation'):
            sqlitecur.execute("""
          CREATE TABLE questioninformation(id bigint ,
						schoolyear bigint ,
						assessmentprogramid bigint ,
						testingprogramid bigint ,
						reportcycle text ,	
        ...
        cursor1.execute("""
            SELECT id, schoolyear, assessmentprogramid, testingprogramid, reportcycle, 
       contentareaid, gradeid, externaltestid, taskvariantexternalid, 
       questiondescription, creditpercent, comment, activeflag, batchuploadid, 
       createduser, modifieduser, createddate, modifieddate
	  FROM questioninformation
	  where assessmentprogramid=%s and schoolyear=%s
        """ % (options.assessment, options.year))
        ...
        for row in cursor1:
            ...
            id, schoolyear, assessmentprogramid, testingprogramid, reportcycle, 
            contentareaid, gradeid, externaltestid, taskvariantexternalid, 
            questiondescription, creditpercent, comment, activeflag, batchuploadid, 
            createduser, modifieduser, createddate, modifieddate)
			VALUES (?, ?, ?, ?, ?,?, ?, ?, ?, 
...
def main():
    ...
    if "student" in options.tables or "all" in options.tables:
        ...
        #extract_student_tests(options, db, sqliteconn)
        extract_student_tests_inactive(options, db, sqliteconn)
        ...
        extract_student_tests_interim(options, db, sqliteconn)
        extract_student_responses_interim(options, db, sqliteconn)
        extract_tests_interim(options, db, sqliteconn)
    ...
    if "reports" in options.tables or "all" in options.tables:
        ...
        extract_interimstudentreport(options, db, sqliteconn)
        extract_studentreportquestioninfo(options, db, sqliteconn)
        extract_questioninformation(options, db, sqliteconn)
    ...
    if "score" in options.tables or "all" in options.tables:
        ...
        extract_scoringassignmentstudent(options, db, sqliteconn)
        extract_scoringassignment(options, db, sqliteconn)
        extract_scoringassignmentscorer(options, db, sqliteconn)
        #extract_ccqscore(options, db, sqliteconn)
        #extract_ccqscoreitem(options, db, sqliteconn)
        extract_studentstestscore(options, db, sqliteconn)
    ...
    if "reference" in options.tables or "all" in options.tables:
        ...
        extract_enrollmentcontentarea(options, db, sqliteconn) 
